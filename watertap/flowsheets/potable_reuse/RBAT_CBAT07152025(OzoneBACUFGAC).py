...
def build_nonRO(working_directory=None, state=None, solute_list=None, effluent_type=None, LRVO3_req=None, LRVClvirus_req=None, LRVClgiardia_req=None):
    ...
    non_RO.s04 = Arc(source=non_RO.GAC.treated, destination=m.fs.treated_nonRO.inlet)
...
def set_operating_conditions(model, solute_list=None, treatment_train=None, system_capacity=None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR
        ...
        # non_RO.GAC.replacement_frequency[0].fix(0.5)
...
def optimize_operation(m, state = None, effluent_type = None, treatment_train = None):
    if treatment_train == "CBAT": #non-RO
        ...
        # non_RO.GAC.replacement_frequency[0].unfix()
        # non_RO.GAC.replacement_frequency[0].setlb(0.01)
        # non_RO.GAC.replacement_frequency[0].setub(5.0)
        ...
        # if hasattr(non_RO.GAC, "replacement_freq"):
        #     non_RO.GAC.del_component("replacement_freq")
        # non_RO.GAC.replacement_freq = Constraint(
        #     m.fs.time,
        #     rule=lambda b, t: non_RO.GAC.replacement_frequency[t] ==
        #                       pyunits.convert(((non_RO.GAC.required_BV[t] * non_RO.GAC.EBCT[t] * pyunits.day) / (
        ...
        #             )
        #             * (
        #                     292000 * ((TOC_safe / (1 * pyunits.mg / pyunits.L)) ** (-1.49))
        #             )
        #             / 24962.88
        #     )
