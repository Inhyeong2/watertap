...
def set_operating_conditions(m, solute_list=None, treatment_train=None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR
        ...
        # so the same recovery needs ~half the pressure. 40 bar (the flat-sheet guess)
        # over-drives recovery past ~0.9 and the RO fails to initialize; 20 bar gives a
        # comparable starting recovery (~0.56 at TDS 0.5-1 kg/m3) and initializes cleanly.
        RO_main.pump.outlet.pressure[0].fix(20*101325)
...
def optimize_operation(m, state = None, effluent_type = None, treatment_train = None):
    if treatment_train == "CBAT": #non-RO
        ...
        # (~0.05 m/s at 79% recovery). Bounding the exit velocity caps recovery to a realistic
        # single-stage range (v_exit>=0.1, v_in<=0.3 -> recovery<=~0.67).
        ...
        if hasattr(RO_main.RO, "eq_max_design_flux"):
            RO_main.RO.del_component("eq_max_design_flux")
        RO_main.RO.eq_max_design_flux = Constraint(
            expr=RO_main.RO.mixed_permeate[0].flow_vol_phase["Liq"]
            <= max_design_flux * RO_main.RO.area
        )
