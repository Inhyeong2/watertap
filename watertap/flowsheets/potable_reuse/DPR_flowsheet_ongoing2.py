...
def set_operating_conditions(m, flow_vol=10 * 0.0438, solute_list=None, treatment_train=None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR
        ...
        m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq","TDS"] = m.fs.feed.properties[0].flow_mass_comp["tds"].value
        ...
        op_pressure, wt_recovery = calculate_operating_pressure_and_recovery(feed_state_block=m.fs.tb_pre_main.properties_out[0])
        ...
        RO_main.RO.recovery_vol_phase[0, "Liq"].fix(wt_recovery)
