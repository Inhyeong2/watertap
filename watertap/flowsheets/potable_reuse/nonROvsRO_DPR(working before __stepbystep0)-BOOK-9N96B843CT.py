...
def build_RO(working_directory=None, solute_list=None): # TODO: [defalut] solute_list is identical to that in build_nonRO
    ...
    RO_pre.s02 = Arc(source=RO_pre.UF.treated, destination=RO_pre.img_sep.inlet)
    m.fs.s_sep_tb1 = Arc(source=RO_pre.img_sep.treated, destination=m.fs.tb_pre_main.inlet)

    m.fs.s_UFby = Arc(source=RO_pre.UF.byproduct, destination=m.fs.byproduct_UF.inlet)
    m.fs.s_sepby = Arc(source=RO_pre.img_sep.byproduct, destination=m.fs.byproduct_imgsep.inlet)
...
def set_operating_conditions(model, is_ro=False):
    ...
    if not is_ro:  # non-RO DPR
        ...
        RO_pre.img_sep.load_parameters_from_database(use_default_removal=True)
