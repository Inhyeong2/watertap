...
def main(erd_type=ERDtype.pump_as_turbine):
    ...
    # calculate_feed_state(m, feed_salinity=5, flow_vol=0.5)
    # for op in np.linspace(0, 1, num=10):
    #     try:
    #         initialize_system(m, overpressure=op)
...
def set_operating_conditions(
        m, flow_vol = 10 * pyunits.Mgallons / pyunits.day,
    ...
    # print(m.fs.feed.outlet.pressure[0].value)
    m.fs.feed.properties[0].temperature.fix(273.15 + 25)  # feed temperature [K]
    m.fs.feed.properties[0].pressure_osm_phase[...] #touching

    # iscale.set_scaling_factor(m.fs.feed.properties[0].flow_vol_phase["Liq"], 1 / value(pyunits.convert(flow_vol / pyunits.Mgallons * pyunits.day, to_units=pyunits.dimensionless)))
    # iscale.set_scaling_factor(m.fs.feed.properties[0].conc_mass_phase_comp["Liq","H2O"], 1 / 1000)
    # iscale.set_scaling_factor(m.fs.feed.properties[0].conc_mass_phase_comp["Liq","TDS"], 1 / value(pyunits.convert(feed_salinity / pyunits.kg * pyunits.m ** 3, to_units=pyunits.dimensionless)))

    calculate_feed_state(m, feed_salinity, flow_vol)

    # print(f"flow_mass_phase_comp of H2O (feed): {m.fs.feed.properties[0].flow_mass_phase_comp['Liq','H2O'].value:.2f} kg/s")
    # print(f"flow_mass_phase_comp of TDS (feed): {m.fs.feed.properties[0].flow_mass_phase_comp['Liq','TDS'].value:.2f} kg/s")
    # print(f"mass_frac_phase_comp of TDS (feed): {m.fs.feed.properties[0].mass_frac_phase_comp['Liq', 'TDS'].value:.2f} ")
    # print(f"conc_mass_phase_comp of H2O (feed): {m.fs.feed.properties[0].conc_mass_phase_comp['Liq', 'H2O'].value:.2f} kg/m3")
    # print(f"conc_mass_phase_comp of TDS (feed): {m.fs.feed.properties[0].conc_mass_phase_comp['Liq', 'TDS'].value:.2f} kg/m3")
    # print(f"flow_vol_phase: {m.fs.feed.properties[0].flow_vol_phase['Liq'].value:.2f} m3/s")
    # print(f"Osmotic pressure of feed: {m.fs.feed.properties[0].pressure_osm_phase['Liq'].value * 1e-5:.2f} bar")

    # Main pump
    m.fs.P1.efficiency_pump.fix(0.80)
    m.fs.P1.control_volume.properties_out[0].pressure.setub(max_operating_pressure)
    ...
    # )
    # m.fs.P1.outlet.pressure[0].fix(op_pressure)
    # print(f"Operating pressure set to {op_pressure * 1e-5:.2f} bar")
    ...
    # m.fs.RO.mixed_permeate[0].pressure.fix(101325)
    m.fs.RO.feed_side.velocity[0, 0].fix(0.25)

    m.fs.RO.length.setlb(6)
    m.fs.RO.length.setub(10)

    feed_side_area_setub = 2*value(pyunits.convert(0.0438 * flow_vol / pyunits.Mgallons * pyunits.day, to_units=pyunits.dimensionless))/m.fs.RO.feed_side.velocity[0, 0].value
    width_setub = feed_side_area_setub / 0.85 / 0.001

    print(f"Area set to {feed_side_area_setub:.2f}")
    print(f"Width to {width_setub:.2f}")

    m.fs.RO.feed_side.area.setub(None)
    m.fs.RO.width.setub(None)
    m.fs.RO.area.setub(None)
    #
    # m.fs.RO.feed_side.area.setub(feed_side_area_setub * 100000)
    # m.fs.RO.width.setub(width_setub * 100000)
    # m.fs.RO.area.setub(width_setub * 100000 * 10)

    m.fs.RO.width.fix(width_setub)
    m.fs.RO.width.unfix()
...
def initialize_system(m, recovery=0.7, overpressure=0.15):
    ...
    # Main pump
    # feed_osm_pressure = m.fs.feed.properties[0].pressure_osm_phase['Liq'].value
    # print(f"Feed osmotic pressure: {feed_osm_pressure:.2f} Pa")

    op_pressure = m.fs.P1.control_volume.properties_out[0].pressure.value
    feed_pressure = m.fs.feed.properties[0].pressure.value

    iscale.set_scaling_factor(m.fs.P1.control_volume.work, 1 / (
                (op_pressure - feed_pressure) * m.fs.feed.properties[0].flow_vol_phase['Liq'].value /
                m.fs.P1.efficiency_pump[0].value))
    print(f"Scaling factor for P1 work: {1/((op_pressure-feed_pressure) * m.fs.feed.properties[0].flow_vol_phase['Liq'].value / m.fs.P1.efficiency_pump[0].value)}")

    # print(f"DeltaP before pump initialization {m.fs.P1.control_volume.deltaP[0].value * 1e-5:.2f} bar")
    m.fs.P1.initialize()
    # print(f"DeltaP after pump initialization {m.fs.P1.control_volume.deltaP[0].value * 1e-5:.2f} bar")
    # m.fs.P1.pprint()
    ...
    calculate_variable_from_constraint(m.fs.RO.width, m.fs.RO.eq_area)
    ...
    iscale.set_scaling_factor(m.fs.RO.feed_side.properties_out[0].flow_mass_phase_comp["Liq", "TDS"],
                              1 / m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "TDS"].value / (1 - recovery)
                              )
    ...
    iscale.set_scaling_factor(m.fs.RO.permeate_side[0, 0].flow_mass_phase_comp["Liq", "TDS"],
                              1 / m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "TDS"].value / recovery / 100
                              )
    ...
    iscale.set_scaling_factor(
        m.fs.RO.mixed_permeate[0].flow_mass_phase_comp["Liq", "TDS"], 1 / m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "TDS"].value / recovery / 100
    )
    ...
    m.fs.RO.initialize()
    # solve(m.fs, checkpoint="solve flowsheet after initializing RO")
