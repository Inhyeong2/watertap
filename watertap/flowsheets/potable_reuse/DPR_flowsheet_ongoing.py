...
def main(working_directory="local", state="FL", treatment_train="RBAT", effluent_type="TERTIARY"):
...
def build_nonRO(working_directory=None, state=None, solute_list=None, effluent_type=None, LRVO3_req=None, LRVClvirus_req=None, LRVClgiardia_req=None):
    ...
    # m.fs.system_capacity = Var(initialize=value(system_capacity), domain=pyo.PositiveReals,
    #                            units=pyunits.Mgallons / pyunits.day)

    # m.fs.system_capacity = system_capacity
...
def build_RO(working_directory=None, state=None, solute_list=None, effluent_type=None, LRVO3_req=None, LRVClvirus_req=None, LRVClgiardia_req=None):
    # flowsheet set up
    m = ConcreteModel()
    if working_directory == None:
        working_directory = os.path.join(
    ...
    # RO_pre.s03 = Arc(source=RO_pre.UF.treated, destination=m.fs.treated_RO.inlet)
...
def set_operating_conditions(model, solute_list=None, treatment_train=None):
    # feed
    ...
    model.fs.feed.flow_vol[0].fix((1+9.9*1) * pyunits.Mgallons / pyunits.day)
    ...
    if treatment_train == "CBAT":  # non-RO DPR
        ...
        non_RO.Ozone.contact_time[0].fix(5 * pyunits.min)
        ...
        non_RO.Cl.contact_time[0].fix(30 * pyunits.min)
        ...
        RO_pre.Ozone.contact_time[0].fix(5 * pyunits.min)

        RO_pre.BAF.load_parameters_from_database(use_default_removal=True)
        RO_pre.BAF.EBCT[0].fix(20 * pyunits.minute)
        ...
        iscale.set_variable_scaling_from_current_value(RO_pre, descend_into=False)
...
#         # RO_post = m.fs.RO_post
        ...
        # Initialize RO_main
        RO_main.RO.feed_side.properties_in[0].flow_mass_phase_comp["Liq", "H2O"] = value(
            model.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"]
        )
        RO_main.RO.feed_side.properties_in[0].flow_mass_phase_comp["Liq", "TDS"] = value(
            model.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"]
        )
        ...
        for solute in specific_solutes:
            model.fs.tb_main_post.properties_out[0].flow_mass_comp[solute].set_value(
                (1 - specific_solutes[solute])
                * RO_main.RO.recovery_vol_phase[0, "Liq"].value
                * model.fs.tb_pre_main.properties_in[0].flow_mass_comp[solute].value
            )
        ...
        for solute in filtered_solute_list:
            model.fs.tb_main_post.properties_out[0].flow_mass_comp[solute].set_value(0)
        ...
        propagate_state(model.fs.s_tb2_prod)
...
def optimize_operation(m, state = None, effluent_type = None, treatment_train = None):
    if treatment_train == "CBAT": #non-RO
        ...
        if state == "CA":
            ...
            # # Constraint: Residual O3 conc

            # CTO3 = 2.47 * non_RO.Ozone.config.LRVO3_required * pyunits.mg * pyunits.min / pyunits.L
            # if hasattr(non_RO, "O3_contact_time"):
            #     non_RO.del_component("O3_contact_time")

            # non_RO.O3_contact_time = Constraint(
            #     m.fs.time,
            #     rule=lambda b, t: 1 - pyunits.convert(
            #         (CTO3 / ((0.704 * non_RO.Ozone.O3toTOC[0] + 1.136) * pyunits.mg / pyunits.L) - 0.5 * pyunits.min) *
            #         non_RO.Ozone.kO3[t], to_units=pyunits.dimensionless)
            #
            #                       >= pyo.exp(
            #         pyunits.convert(
            #             -non_RO.Ozone.kO3[t] * (non_RO.Ozone.contact_time[t] - 0.5 * pyunits.min),
            #             to_units=pyunits.dimensionless))
            # )


            # non_RO.residual_O3 = Constraint(
            #     m.fs.time,
            #     rule=lambda b, t: ((0.704 * non_RO.Ozone.O3toTOC[t] + 1.136) * (pyunits.mg / pyunits.L)) * pyo.exp(
            #         pyunits.convert(
            #             -non_RO.Ozone.kO3[t] * (non_RO.Ozone.contact_time[t] - 0.5 * pyunits.min),
            #             to_units=pyunits.dimensionless)
            #     ) >= 0.2 * pyunits.mg / pyunits.L
            # )
        ...
        #             )
        #             * (
        #                     292000 * (pyunits.convert(non_RO.GAC.properties_in[t].conc_mass_comp["toc"],
        #                                               to_units=pyunits.mg / pyunits.L) / (
        #                                           1 * pyunits.mg / pyunits.L)) ** (-1.49)
        #             )
        #             / 24962.88
        #     )
        # )
...
def add_costing(model, treatment_train=None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR costing
        non_RO = model.fs.non_RO
        model.fs.zo_costing = ZeroOrderCosting(case_study_definition=source_file)

        # non-RO DPR: cost of each unit process
        non_RO.Ozone.costing = UnitModelCostingBlock(flowsheet_costing_block=model.fs.zo_costing)
        non_RO.BAF.costing = UnitModelCostingBlock(flowsheet_costing_block=model.fs.zo_costing)
        non_RO.UF.costing = UnitModelCostingBlock(flowsheet_costing_block=model.fs.zo_costing)
        non_RO.GAC.costing = UnitModelCostingBlock(flowsheet_costing_block=model.fs.zo_costing)
        non_RO.UV_AOP.costing = UnitModelCostingBlock(flowsheet_costing_block=model.fs.zo_costing)
        non_RO.Cl.costing = UnitModelCostingBlock(flowsheet_costing_block=model.fs.zo_costing)

        # Aggregate unit level costs and calculate overall process costs
        model.fs.zo_costing.cost_process()

        feed_flowrate = model.fs.feed.flow_vol[0]
        model.fs.zo_costing.add_electricity_intensity(feed_flowrate)
        model.fs.specific_energy_intensity = Expression(
            expr=(model.fs.zo_costing.electricity_intensity),
        ...
        model.fs.water_recovery_revenue = Expression(
            expr=(
                -1 * model.fs.zo_costing.utilization_factor
                * model.fs.zo_costing.recovered_water_cost
                * pyunits.convert(
                    ...
                    to_units=pyunits.m**3 / model.fs.zo_costing.base_period,
                )
            ),
        ...
        def total_capital_cost(b):
            return pyunits.convert(
                model.fs.zo_costing.total_capital_cost, to_units=pyunits.USD_2020
            )

        @model.fs.Expression(doc="Total operating cost of the non-RO DPR treatment train")
        def total_operating_cost(b):
            return pyunits.convert(
                model.fs.zo_costing.total_fixed_operating_cost,
                to_units=pyunits.USD_2020 / pyunits.year,
            ) + pyunits.convert(
                model.fs.zo_costing.total_variable_operating_cost,
                to_units=pyunits.USD_2020 / pyunits.year,
            )
    ...
    #     m.fs.ro_costing = WaterTAPCosting()
        ...
        RO_pre.Ozone.costing = UnitModelCostingBlock(flowsheet_costing_block=model.fs.zo_costing)
        RO_pre.BAF.costing = UnitModelCostingBlock(flowsheet_costing_block=model.fs.zo_costing)
        RO_pre.UF.costing = UnitModelCostingBlock(flowsheet_costing_block=model.fs.zo_costing)
        ...
        model.fs.specific_energy_intensity = Expression(
                expr=(
                        model.fs.zo_costing.electricity_intensity
...
def initialize_costing(model, treatment_train=None):
    if treatment_train == "CBAT":
        model.fs.zo_costing.initialize()
    elif treatment_train == "RBAT":
        model.fs.zo_costing.initialize()
...
def display_results(m, treatment_train=None):
    ...
    m.fs.non_RO.Ozone.properties_in.report()
    m.fs.non_RO.Ozone.properties_in[0].display()
    print(hasattr( m.fs.non_RO.Ozone.properties_in[0], "temperature"))
...
def display_cost(m, treatment_train=None):
    if treatment_train == "CBAT":
        ...
        for u in m.fs.zo_costing._registered_unit_costing:
            unit_name = u.parent_block().local_name
            print(
                f"{unit_name} capital cost: {value(pyunits.convert(u.capital_cost*m.fs.zo_costing.total_investment_factor, to_units=pyunits.MUSD_2020)):.4f} $")
            total_unit_capex_cal += value(pyunits.convert(u.capital_cost*m.fs.zo_costing.total_investment_factor, to_units=pyunits.MUSD_2020))

        #todo should consider total_investment_factor

        # for u in m.fs.ro_costing._registered_unit_costing:
        #     unit_name = u.parent_block().local_name
        #     print(
        #         f"{unit_name} capital cost: {value(pyunits.convert(u.capital_cost, to_units=pyunits.USD_2020)):.3f} $")
