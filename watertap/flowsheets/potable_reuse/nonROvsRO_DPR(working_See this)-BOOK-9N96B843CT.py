...
from watertap.flowsheets.nonRO_DPR import ImaginarySeparatorZO
...
def build_nonRO(working_directory=None):
    ...
    m.fs.s_non_RO_treated = Arc(source=non_RO.Cl.treated, destination=m.fs.treated_nonRO.inlet)
...
def build_RO(working_directory=None, solute_list=None): # TODO: [defalut] solute_list is identical to that in build_nonRO
    ...
    #         solute_list=["tds", "toc"], # tds should be included here
    #         mw_data={
    #             "H2O": 0.018,
    #             "tds": 0.05844,
    #             "toc": 0.060,  # molecular weight of solute defined in specific_solutes H3BO3=61.83g/mol, BO33-=58.81g/mol
    #         },
    #         diffus_calculation=DiffusivityCalculation.none,
    #         diffusivity_data={("Liq", "tds"): 1e-09, ("Liq", "toc"): 1e-12},
    #     )
    ...
    RO_pre.img_sep = ImaginarySeparatorZO(property_package=m.fs.prop_zo, database=m.db)

    m.fs.byproduct_UF = Product(property_package=m.fs.prop_zo)

    # Translator Block from ZO to MCAS (before imaginary separator)
    m.fs.tb_zo_mcas = Translator(
        inlet_property_package=m.fs.prop_zo,
        outlet_property_package=m.fs.prop_mcas,
    )

    # Imaginary separator (after translator block)
    m.fs.sep_img = Separator(
        property_package=m.fs.prop_mcas,
        outlet_list=["tb_pre_main", "mixer_img"],
    )

    # Add split fraction constraints for water and specific solutes (TDS, TOC)
    @m.fs.sep_img.Constraint(m.fs.prop_mcas.component_list)
    def eq_split_fractions(b, j):
        if j == "tds" or j in specific_solutes:
            return b.split_fraction[
                0, "tb_pre_main", "Liq", j] == 0.99999  # Most tds and specific solutes to tb_pre_main
        elif j == "H2O":
            return b.split_fraction[0, "tb_pre_main", "Liq", j] == 0.99999  # Most H2O to tb_pre_main
        else:
            return b.split_fraction[0, "tb_pre_main", "Liq", j] == 0.00001  # Small amount of other solutes

    # Add split fraction constraints for water and solutes
    @m.fs.sep_img.Constraint(m.fs.prop_mcas.component_list)
    def eq_split_fractions(b, j):
        if j == "TDS":
            return b.split_fraction[0, "tb_pre_main", "Liq", j] == 0.99999  # Most tds to tb_pre_main
        elif j == "H2O":
            return b.split_fraction[0, "tb_pre_main", "Liq", j] == 0.99999  # Most H2O to tb_pre_main
        else:
            return b.split_fraction[0, "tb_pre_main", "Liq", j] == 0.00001  # Small amount of other solutes
    ...
    @m.fs.tb_main_post.Constraint(["H2O", "TDS"])
    def eq_flow_mass_comp(blk, j):
        if j == "TDS":
            return (
    ...
    # connections (RO DPR)
    m.fs.s_RO_feed = Arc(source=m.fs.feed.outlet, destination=RO_pre.Cl.inlet)
    RO_pre.s01 = Arc(source=RO_pre.Cl.treated, destination=RO_pre.UF.inlet)
    m.fs.s_RO_pre = Arc(source=RO_pre.UF.treated, destination=m.fs.tb_pre_main.inlet)
    m.fs.s_RO_main = Arc(source=m.fs.tb_pre_main.outlet, destination=RO_main.S1.inlet)
    ...
    m.fs.s_permeate = Arc(source=RO_main.RO.permeate, destination=m.fs.tb_main_post.inlet)
    m.fs.s_RO_post = Arc(source=m.fs.tb_main_post.outlet, destination=RO_post.UV_AOP.inlet)
    m.fs.s_RO_treated = Arc(source=RO_post.UV_AOP.treated, destination=m.fs.treated_RO.inlet)
    ...
    iscale.calculate_scaling_factors(m.fs.RO_pre)
...
def initialize_system(model, is_ro=False):
    ...
    if not is_ro:  # non-RO DPR
        ...
        # Initialize RO_pre in RO DPR
        propagate_state(model.fs.s_RO_feed)
        seq.run(RO_pre, lambda u: u.initialize())
        propagate_state(model.fs.s_RO_pre)  # Transfer state from RO_pre to RO_main

        # Initialize RO_main in RO DPR
        ...
        propagate_state(model.fs.s_RO_main)
...
def display_results_RO_DPR(m):
    ...
    sys_water_recovery = (
            m.fs.treated_RO.properties[0].flow_mass_phase_comp["Liq", "H2O"]()
            / m.fs.feed.flow_mass_comp[0, "H2O"]()
    )
    print(f"System water recovery: {sys_water_recovery * 100 : .3f}%")
    sys_tds_recovery = (
            m.fs.treated_RO.properties[0].flow_mass_phase_comp["Liq", "TDS"]()
