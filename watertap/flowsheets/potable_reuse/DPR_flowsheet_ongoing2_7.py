...
def main(state="FL", treatment_train="RBAT", effluent_type="TERTIARY"):
    ...
    # print(f"Operating pressure (pump): {m.fs.RO_main.pump.control_volume.properties_out[0].pressure.value/1e5:.2f} bar")
    # print(f"Recovery: {m.fs.RO_main.RO.recovery_vol_phase[0, 'Liq'].value:.2f}")
    # print(f"Membrane area: {m.fs.RO_main.RO.area.value:.2f} m2")
...
def set_operating_conditions(m, flow_vol=10 * 0.0438, solute_list=None, treatment_train=None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR
        ...
        # m.fs.tb_main_post.properties_out[0].flow_vol.fix()
...
def add_costing(m, treatment_train=None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR costing
        ...
        # m.fs.ro_costing.land_cost_percent_FCI = value(m.fs.zo_costing.land_cost_percent_FCI)
        # m.fs.ro_costing.working_capital_percent_FCI = value(m.fs.zo_costing.working_capital_percent_FCI)
        # m.fs.ro_costing.salaries_percent_FCI = value(m.fs.zo_costing.salaries_percent_FCI)
        # m.fs.ro_costing.benefit_percent_of_salary = value(m.fs.zo_costing.benefit_percent_of_salary)
        # m.fs.ro_costing.maintenance_costs_percent_FCI = value(m.fs.zo_costing.maintenance_costs_percent_FCI)
        # m.fs.ro_costing.laboratory_fees_percent_FCI = value(m.fs.zo_costing.laboratory_fees_percent_FCI)
        # m.fs.ro_costing.insurance_and_taxes_percent_FCI = value(m.fs.zo_costing.insurance_and_taxes_percent_FCI)
        #
        # m.fs.ro_costing.TIC = value(m.fs.zo_costing.TIC)
        # m.fs.ro_costing.total_investment_factor = value(m.fs.zo_costing.total_investment_factor)
