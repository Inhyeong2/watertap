...
def build_RO(working_directory="local", state=None, solute_list=None, effluent_type=None, treatment_train=None, LRVO3_req=None, LRVClvirus_req=None, LRVClgiardia_req=None):
    ...
    # RO DPR components
    RO_pre.Ozone = OzoneDPRZO(property_package=m.fs.prop_zo, database=m.db, state=state, effluent_type=effluent_type,
                              LRVO3_required=LRVO3_req)
    ...
    RO_pre.UV_AOP = UVAOPDPRZO(property_package=m.fs.prop_zo, database=m.db, treatment_train=treatment_train)
    RO_pre.Cl = ChlorinationDPRZO(property_package=m.fs.prop_zo, database=m.db, LRVCl_required_for_virus=LRVClvirus_req, LRVCl_required_for_giardia=LRVClgiardia_req)
    ...
    m.fs.s_tb2_UVAOP = Arc(source=m.fs.tb_main_post.outlet, destination=RO_pre.UV_AOP.inlet)
    m.fs.s_UVAOP_Cl = Arc(source=RO_pre.UV_AOP.treated, destination=RO_pre.Cl.inlet)
    m.fs.s_Cl_prod = Arc(source=RO_pre.Cl.treated, destination=m.fs.treated_RO.inlet)
...
def set_operating_conditions(m, flow_vol=10 * 0.0438, solute_list=None, treatment_train=None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR
        ...
        feed_side_area_guess = flow_vol / RO_main.RO.feed_side.velocity[0, 0].value
        ...
        RO_pre.UV_AOP.load_parameters_from_database(use_default_removal=True)
        RO_pre.UV_AOP.hydrogen_peroxide_dose[0].fix(5)
        RO_pre.UV_AOP.chloramine_dose[0].fix(2.5)

        RO_pre.Cl.load_parameters_from_database(use_default_removal=True)
        RO_pre.Cl.contact_time[0].fix(30)
...
def scale_system(m, treatment_train = None):
    ...
    if treatment_train == "CBAT":
        ...
        #UV-AOP
        for prop_blk in [RO_pre.UV_AOP.properties_in[0], RO_pre.UV_AOP.properties_treated[0]]:
            for j in prop_blk.flow_mass_comp:
                val = value(m.fs.tb_main_post.properties_out[0].flow_mass_comp[j])
                iscale.set_scaling_factor(prop_blk.flow_mass_comp[j], 1 / val)
        iscale.set_scaling_factor(RO_pre.UV_AOP.uv_dose[0], 2e-3)
        iscale.set_scaling_factor(RO_pre.UV_AOP.hydrogen_peroxide_dose[0], 1/5)
        iscale.set_scaling_factor(RO_pre.UV_AOP.chloramine_dose[0], 1 / 2.5)
        iscale.set_scaling_factor(RO_pre.UV_AOP.energy_electric_flow_vol_inlet, 10)

        #Chlorimation
        for prop_blk in [RO_pre.Cl.properties_in[0], RO_pre.Cl.properties_treated[0]]:
            for j in prop_blk.flow_mass_comp:
                val = value(RO_pre.UV_AOP.properties_treated[0].flow_mass_comp[j])
                iscale.set_scaling_factor(prop_blk.flow_mass_comp[j], 1 / val)
        iscale.set_scaling_factor(RO_pre.Cl.energy_electric_flow_vol_inlet, 2e4)  # todo: check this value
        # iscale.set_scaling_factor(non_RO.Cl.initial_chlorine_demand[0], 1)
        iscale.set_scaling_factor(RO_pre.Cl.contact_time[0], 1 / 30)
        iscale.set_scaling_factor(RO_pre.Cl.chlorine_decay_rate[0], 10)
        iscale.set_scaling_factor(RO_pre.Cl.chlorine_dose[0], 1)
...
def initialize_system(m, solute_list = None, treatment_train = None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR
        ...
        RO_pre.UV_AOP.initialize()

        propagate_state(m.fs.s_UVAOP_Cl)
        RO_pre.Cl.initialize()
...
def optimize_operation(m, state = None, effluent_type = None, treatment_train = None):
    if treatment_train == "CBAT": #non-RO
        ...
        RO_main.RO.recovery_vol_phase[0, "Liq"].unfix()
        RO_main.RO.recovery_vol_phase[0, "Liq"].setub(0.80)
        ...
        #UV-AOP
        RO_pre.UV_AOP.hydrogen_peroxide_dose[0].unfix()
        RO_pre.UV_AOP.hydrogen_peroxide_dose[0].setlb(2)
        RO_pre.UV_AOP.hydrogen_peroxide_dose[0].setub(10)

        RO_pre.UV_AOP.uv_dose[0].unfix()
        RO_pre.UV_AOP.uv_dose[0].setlb(300)  # reg
        RO_pre.UV_AOP.uv_dose[0].setub(3500)

        RO_pre.Cl.contact_time[0].unfix()
        RO_pre.Cl.contact_time[0].setlb(30)
        RO_pre.Cl.contact_time[0].setub(60)
...
def add_costing(m, treatment_train=None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR costing
        ...
        RO_pre.UV_AOP.costing = UnitModelCostingBlock(flowsheet_costing_block=m.fs.zo_costing)
        RO_pre.Cl.costing = UnitModelCostingBlock(flowsheet_costing_block=m.fs.zo_costing)
        ...
        m.fs.ro_costing.reverse_osmosis.factor_membrane_replacement.fix(
            0.2 * m.fs.ro_costing.utilization_factor.value)  # utilization factor should be considered
...
def display_results(m, treatment_train=None):
    ...
    if treatment_train == "RBAT":
        ...
        print(
            f"Flow rate of UV-AOP: {value(pyunits.convert(m.fs.RO_pre.UV_AOP.properties_treated[0].flow_vol, to_units=pyunits.Mgallons / pyunits.day)): .3f} MGD")
        print(
            f"Flow rate of Cl: {value(pyunits.convert(m.fs.RO_pre.Cl.properties_treated[0].flow_vol, to_units=pyunits.Mgallons / pyunits.day)): .3f} MGD")
        print(
        ...
        print(
            f"Operating conditions (UV-AOP): "
            f"H2O2 dose = {value(pyunits.convert(m.fs.RO_pre.UV_AOP.hydrogen_peroxide_dose[0], to_units=pyunits.mg /pyunits.L)):.3f} mg/L, "
            f"UV dose = {value(pyunits.convert(m.fs.RO_pre.UV_AOP.uv_dose[0], to_units=pyunits.mJ /pyunits.cm **2)):.3f} mJ/cm2"
        )

        print(
            f"Operating conditions (Cl): "
            f"Cl dose = {value(pyunits.convert(m.fs.RO_pre.Cl.chlorine_dose[0], to_units=pyunits.mg / pyunits.L)):.3f} mg/L, "
            f"Contact time = {value(pyunits.convert(m.fs.RO_pre.Cl.contact_time[0], to_units=pyunits.min)):.3f} min, "
            f"Residual Cl = {value(pyunits.convert(m.fs.RO_pre.Cl.chlorine_dose[0] + m.fs.RO_pre.Cl.initial_chlorine_demand[0], to_units=pyunits.mg / pyunits.L)) * value(pyo.exp(-m.fs.RO_pre.Cl.chlorine_decay_rate[0] * value(pyunits.convert(m.fs.RO_pre.Cl.contact_time[0], to_units=pyunits.hour)))) :.3f} mg/L"
        )
...
def display_cost(m, treatment_train=None):
    if treatment_train == "CBAT":
        ...
        total_fixed_operating_cost = (value(pyunits.convert(m.fs.zo_costing.total_fixed_operating_cost + m.fs.ro_costing.total_fixed_operating_cost
                                                            - m.fs.ro_costing.aggregate_fixed_operating_cost, #todo: this should be considered as flow (only RO has this value)
                                                            to_units=pyunits.MUSD_2020 / pyunits.year)) )
        ...
        # Comparison with calculated values
        total_fixed_operating_cost_cal = value(pyunits.convert(m.fs.zo_costing.aggregate_fixed_operating_cost + m.fs.zo_costing.maintenance_labor_chemical_operating_cost
                                                               + m.fs.ro_costing.aggregate_fixed_operating_cost * 0 #todo: this should be considered as flow (only RO has this value)
                                                               + m.fs.ro_costing.maintenance_labor_chemical_operating_cost,
                                                                to_units=pyunits.MUSD_2020 / pyunits.year))
                    #                       + value(
        ...
        total_flow_cost_cal = value(
            pyunits.convert(
                (sum(m.fs.zo_costing.aggregate_flow_costs[flow] for flow in m.fs.zo_costing.used_flows)
                 + m.fs.ro_costing.aggregate_flow_costs["electricity"])
        ...
        # # Add RO_pre units
        # unit_blocks.update({
        #     f"RO_pre.{name}": block for name, block in m.fs.RO_pre.component_map(Block).items()
        #     if isinstance(block, UnitModelBlockData)
        # })
        #
        # # Add RO_main units
        # unit_blocks.update({
        #     f"RO_main.{name}": block for name, block in m.fs.RO_main.component_map(Block).items()
        #     if isinstance(block, UnitModelBlockData)
        # })
        #
        # # # Add RO_post units
        # # unit_blocks.update({
        # #     f"RO_post.{name}": block for name, block in m.fs.RO_post.component_map(Block).items()
        # #     if isinstance(block, UnitModelBlockData)
        # # })
        #
        # total_flow_cost = 0.0
        ...
        print("\n----------Unit Operating Costs (only flow cost) [RO_pre / ZO]----------")
        ...
        unit_blocks_ro_pre = {
            f"RO_pre.{name}": blk for name, blk in m.fs.RO_pre.component_map(Block).items()
            if isinstance(blk, UnitModelBlockData)
        }

        total_flow_cost_ro_pre = 0.0
        for unit_name, unit_block in unit_blocks_ro_pre.items():
            unit_total = 0.0
            print(f"{unit_name}:")
            for ft in flow_types_zo:
                if hasattr(unit_block, ft):
                    ...
                    unit_total += cost
            print(f"  Total flow cost: {unit_total:,.2f} USD/year\n")
            total_flow_cost_ro_pre += unit_total
        print(f"Total flow cost across RO_pre units: {total_flow_cost_ro_pre:,.2f} USD/year")
        ...
        for unit_name, unit_block in unit_blocks_ro_main.items():
            ...
            if unit_name.endswith(".RO"):
                memrep_cost = value(pyunits.convert(
                    m.fs.ro_costing.aggregate_fixed_operating_cost,  # as requested
                    to_units=pyunits.USD_2020 / pyunits.year
                ))
        ...
        total_flow=total_flow_cost_ro_pre+total_flow_cost_ro_main
        ...
        # print(hasattr(m.fs.RO_main.pump, "electricity_cost"))  # True or False
        # print(value(m.fs.RO_main.pump.electricity))  # 값이 0이면 비용도 0
        # print(hasattr(m.fs.RO_main.RO, "electricity"))  # True or False
        # print(value(m.fs.RO_main.RO.electricity))  # 값이 0이면 비용도 0
        # print(hasattr(m.fs.RO_main.ERD, "electricity"))  # True or False
        # print(value(m.fs.RO_main.ERD.electricity))  # 값이 0이면 비용도 0
