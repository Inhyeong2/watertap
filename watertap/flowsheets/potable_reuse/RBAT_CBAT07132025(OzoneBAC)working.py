...
def main(working_directory=None):
    ...
    # print("Before init:", value(m.fs.non_RO.BAF.O3toTOC[0]))
    initialize_system(m, solute_list=solute_list, treatment_train=treatment_train)
    assert_degrees_of_freedom(m, 0)
    # print("After init:", value(m.fs.non_RO.BAF.O3toTOC[0]))
    #
...
def build_nonRO(working_directory=None, state=None, solute_list=None, effluent_type=None, LRVO3_req=None, LRVClvirus_req=None, LRVClgiardia_req=None):
    ...
    non_RO.s02 = Arc(source=non_RO.BAF.treated, destination=m.fs.treated_nonRO.inlet)
...
def optimize_operation(m, state = None, effluent_type = None, treatment_train = None):
    if treatment_train == "CBAT": #non-RO
        ...
        # for GAC add... --> we need to calculate GAC TOC removal as well like thie right above constraints
        # and also we need to add one more constraint for TOC removal equation like
        # (1-removal_toc_BAF)*(1-removal_toc_GAC) == target TOC/inlet TOC
