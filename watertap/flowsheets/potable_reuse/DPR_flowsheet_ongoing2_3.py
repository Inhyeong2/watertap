...
def adaptive_tds_sweep(m, solute_list = None, target_tds=None, initial_tds=15, step_sizes=None):
    print("###### Adaptive TDS Sweep #######")
    if step_sizes is None:
        step_sizes = [5, 2.5, 1, 0.5, 0.25, 0.1]
    ...
    while True:
        ...
        try:
            ...
            solve_RO_initial_conditions(m, current_tds=current_tds, solute_list=solute_list, water_recovery=0.7, TDS_passage=0.01)
            ...
            print(f"Failed at TDS {current_tds} g/L: {str(e)}")
            step_idx += 1
    ...
    if current_tds > target_tds:
        raise RuntimeError(
            f"\n Could not reach the minimum target TDS ({target_tds} g/L). "
...
def solve_RO_initial_conditions(m,
    ...
    over_pressure_estimate = 10 ** (-math.log10(m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value) - 2.5)
