...
def initialize_system(model, is_ro=False, solute_list=None):
    ...
    if not is_ro:  # non-RO DPR
        ...
        propagate_state(model.fs.s_UF_tb1)
        ...
        # RO_main.RO.inlet.flow_mass_phase_comp[0, "Liq", "TDS"] = value(
        ...
        RO_main.RO.feed_side.properties_in[0].temperature = value(
            model.fs.tb_pre_main.properties_out[0].temperature
        )
        RO_main.RO.feed_side.properties_in[0].pressure = value(
            RO_main.P1.control_volume.properties_out[0].pressure
        )
        ...
        propagate_state(model.fs.s_RO_tb2)

        # Translate flow from tb_main_post to RO_post for all solutes
        model.fs.tb_main_post.properties_out[0].flow_mass_comp["H2O"] = value(
            model.fs.tb_main_post.properties_in[0].flow_mass_phase_comp["Liq", "H2O"]
        )
        model.fs.tb_main_post.properties_out[0].flow_mass_comp["tds"] = value(
            model.fs.tb_main_post.properties_in[0].flow_mass_phase_comp["Liq", "TDS"]
        )

        filtered_solute_list = [s for s in solute_list if s not in ["tds", "H2O"]]
        for solute in filtered_solute_list:
            model.fs.tb_main_post.properties_out[0].flow_mass_comp[solute] = 0

        # Propagate state from tb2 to RO_post
        propagate_state(model.fs.s_tb2_UVAOP)
...
def add_costing(model, is_ro=False):
    ...
    if not is_ro:  # non-RO DPR costing
        ...
        model.fs.zo_costing_RO_post = ZeroOrderCosting(case_study_definition=source_file)
        ...
        RO_post.UV_AOP.costing = UnitModelCostingBlock(flowsheet_costing_block=model.fs.zo_costing_RO_post)

        # Aggregate unit level costs and calculate overall process costs
        model.fs.zo_costing_RO_post.cost_process()

        feed_flowrate_post = RO_post.UV_AOP.properties_in[0].flow_vol
        model.fs.zo_costing_RO_post.add_electricity_intensity(feed_flowrate_post)
        ...
        def total_capital_cost(b):
            return pyunits.convert(
                ...
                model.fs.zo_costing_RO_post.total_capital_cost, to_units=pyunits.USD_2020
            )
        ...
        def total_operating_cost(b):
            return pyunits.convert(
                ...
                model.fs.zo_costing_RO_post.total_fixed_operating_cost,
                to_units=pyunits.USD_2020 / pyunits.year,
            ) + pyunits.convert(
                model.fs.zo_costing_RO_post.total_variable_operating_cost,
                to_units=pyunits.USD_2020 / pyunits.year,
            )
