...
import watertap.flowsheets.potable_reuse.DPR_flowsheet_v2_spiral_multistage as dpr
...
def _solve_rbat_at(state, flow_mgd, toc, tds, brine):
    """Build the multi-stage RBAT flowsheet at EXPLICIT feed inputs, optimize LCOW, and return
    (converged, model). A fresh build gives point-local scaling, so it escapes the warm-start
    local optimum a swept sample can get stuck in."""
    LRV, det, st, tt, eff, sol = dpr.DPR_initial_setting(
        state=state, treatment_train="RBAT", effluent_type="TERTIARY")
    m = dpr.build_RO(solute_list=sol, state=st, effluent_type=eff, treatment_train=tt,
                     LRVO3_req=LRV["ozone_crypto_lrv_required"],
                     LRVClvirus_req=LRV["cl2_virus_lrv_required"],
                     LRVClgiardia_req=LRV["cl2_giardia_lrv_required"], n_stages=N_STAGES)
    dpr.set_operating_conditions(m, solute_list=sol, treatment_train=tt)
    m.fs.feed.flow_vol[0].fix(flow_mgd * MGD)
...
def repair_mc_outliers(state, mc_csv, worst_flows, worst_lcow, tol=0.01, verbose=True):
    ...
    from watertap.flowsheets.potable_reuse.DPR_sweep_v2_spiral_multistage import (
        set_up_sensitivity,
    )
    ...
    for idx in idxs:
        ...
        toc, tds, brine = (raw.at[idx, "feed_toc"], raw.at[idx, "feed_tds"],
                           raw.at[idx, "brine_disposal_cost"])
        old = raw.at[idx, "LCOW"]
        bound = we[idx]
        best_lcow, best_m = old, None
        for eps in ENV_MULTISTART_PERTURB:
            try:
                ok, m = _solve_rbat_at(state, flow * (1 + eps), toc, tds, brine)
                lc = value(m.fs.LCOW)
        ...
        if best_m is not None and best_lcow < old - 1e-9:
            for k, v in set_up_sensitivity(best_m, treatment_train="RBAT").items():
                if k in raw.columns:
                    try:
