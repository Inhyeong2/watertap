...
# Tiny relative feed-flow perturbations retried when a point lands on a solver
# knife-edge: IPOPT errors at the exact sampled value but converges a hair away (a
# numerical artifact, not infeasibility -- verified that rounding the inputs recovers
# the same LCOW). The shift is at most 5e-4 relative, so the recovered outputs differ
# from the true point by far less than any meaningful resolution; we record them
# against the original (unperturbed) CSV inputs. The first entry (0.0) is the exact
# value -- most points solve there and never perturb.
_RESCUE_FLOW_PERTURBATIONS = (0.0, 1e-5, 1e-4, 5e-4, 1e-3)
    ...
    for eps in _RESCUE_FLOW_PERTURBATIONS:
        ...
        try:
            ...
            dpr.set_operating_conditions(
                m, solute_list=solute_list, treatment_train=treatment_train
            )
...
def run_monte_carlo(
    ...
    if mc_config is None:
        mc_config = dict(DEFAULT_MC_CONFIG)
    if seed is not None:
        np.random.seed(seed)
...
if __name__ == "__main__":
    # Monte Carlo for CA / CO / FL on the SAME 1000 random input samples (only the
    # state's regulatory requirements -- and therefore LCOW -- differ). One CSV and one
    # PNG per state, written to OUTPUT_DIR/monte_carlo.
