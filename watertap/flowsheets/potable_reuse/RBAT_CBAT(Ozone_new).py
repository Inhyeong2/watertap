...
def main(working_directory=None):
    ...
    m, solute_list = build_nonRO(working_directory="local")
    ...
    initialize_system(m, is_ro=False, solute_list=solute_list)
    # initialize_system(n, is_ro=True, solute_list=solute_list)
    ...
    return m, results_nonRO


def DPR_input(states="CA", treatment_train="CBAT", effluent_type="Tertiary"):
    # Current regulatory frameworks
    LRVvirus_CA = 20
    LRVvirus_CO = 12
    LRVvirus_FL = LRVvirus_AZ = 14

    LRVgiardia_CA = 14
    LRVgiardia_CO = 10
    LRVgiardia_FL = LRVgiardia_AZ = 12

    LRVcrypto_CA = 15
    LRVcrypto_CO = 10
    LRVcrypto_FL = LRVcrypto_AZ = 12

    #Under current regulations
    LRVvirus_UF = 0
    LRVgiardia_UF = LRVcrypto_UF = 4
    LRVvirus_RO = LRVgiardia_RO = LRVcrypto_RO = 2
    LRVvirus_UVAOP = LRVgiardia_UVAOP = LRVcrypto_UVAOP = 6

    LRVvirus_O3 = LRVgiardia_O3 = 6

    Virus criteria
    LRVvirus_(states) <= Ozone
...
def build_nonRO(working_directory=None):
    ...
    non_RO.s01 = Arc(source=non_RO.Ozone.treated, destination=m.fs.treated_nonRO.inlet)
...
#
#     # define blocks
#     RO_pre = m.fs.RO_pre = Block() # include zero-order ozonation and ultrafiltration
...
# #     specific_solutes = {"toc": 0.99, "tss": 0.98} # solutes & rejection ratio (= 1-C_permeate/C_feed)
# #
# #     @m.fs.tb_main_post.Constraint(["H2O", "TDS", *filtered_solute_list])
...
# #             return (
# #                     blk.properties_out[0].flow_mass_comp[j]
# #                     == (1-specific_solutes[j]) * m.fs.RO_main.RO.recovery_vol_phase[0, "Liq"] * m.fs.tb_pre_main.properties_in[0].flow_mass_comp[j]
# #             )
# #         else:
...
#     RO_pre.s01 = Arc(source=RO_pre.Ozone.treated, destination=m.fs.treated_RO.inlet)
# #     RO_pre.s01 = Arc(source=RO_pre.Ozone.treated, destination=RO_pre.UF.inlet)
...
def set_operating_conditions(model, is_ro=False):
    ...
    feed_flow_vol = 0.04381 * pyunits.m ** 3 / pyunits.s  # 1 MGD = 0.04381 m3/s
    feed_conc_mass_toc = 0.01 * pyunits.kg / pyunits.m ** 3  # 1 mg/L = 0.001 kg/m3
    # feed_conc_mass_tss = 0.01 * pyunits.kg / pyunits.m ** 3
    feed_conc_mass_tds = 0.5 * pyunits.kg / pyunits.m ** 3
...
def initialize_system(model, is_ro=False, solute_list=None):
    # initialize feed
    ...
    if not is_ro:  # non-RO DPR
        ...
        propagate_state(model.fs.s_RO_feed)
...
#         non_RO = m.fs.non_RO
#         m.fs.zo_costing_nonRO = ZeroOrderCosting(case_study_definition=source_file)
#
#         # non-RO DPR: cost of each unit process
#         non_RO.Ozone.costing = UnitModelCostingBlock(flowsheet_costing_block=m.fs.zo_costing_nonRO)
#         non_RO.BAF.costing = UnitModelCostingBlock(flowsheet_costing_block=m.fs.zo_costing_nonRO)
#         non_RO.UF.costing = UnitModelCostingBlock(flowsheet_costing_block=m.fs.zo_costing_nonRO)
#         non_RO.GAC.costing = UnitModelCostingBlock(flowsheet_costing_block=m.fs.zo_costing_nonRO)
#         non_RO.UV_AOP.costing = UnitModelCostingBlock(flowsheet_costing_block=m.fs.zo_costing_nonRO)
...
#         m.fs.zo_costing_nonRO.cost_process()
#
#         feed_flowrate = m.fs.feed.flow_vol[0]
#         m.fs.zo_costing_nonRO.add_electricity_intensity(feed_flowrate)
#         m.fs.specific_energy_intensity = Expression(
#             expr=(m.fs.zo_costing_nonRO.electricity_intensity),
...
#             expr=(
#                 -1 * m.fs.zo_costing_nonRO.utilization_factor
#                 * m.fs.zo_costing_nonRO.recovered_water_cost
#                 * pyunits.convert(
#                     m.fs.byproduct_BAF.properties[0].flow_vol
#                     + m.fs.byproduct_UF.properties[0].flow_vol
#                     + m.fs.byproduct_GAC.properties[0].flow_vol,
#                     to_units=pyunits.m**3 / m.fs.zo_costing_nonRO.base_period,
#                 )
#             ),
...
#                 m.fs.zo_costing_nonRO.total_capital_cost, to_units=pyunits.USD_2020
#             )
#
#         @m.fs.Expression(doc="Total operating cost of the non-RO DPR treatment train")
#         def total_operating_cost(b):
#             return pyunits.convert(
#                 m.fs.zo_costing_nonRO.total_fixed_operating_cost,
#                 to_units=pyunits.USD_2020 / pyunits.year,
#             ) + pyunits.convert(
#                 m.fs.zo_costing_nonRO.total_variable_operating_cost,
#                 to_units=pyunits.USD_2020 / pyunits.year,
#             )
#
...
# def initialize_costing(m, is_ro=False):
#     if not is_ro:
#         m.fs.zo_costing_nonRO.initialize()
#     else:
#         m.fs.zo_costing_RO_pre.initialize()
#         m.fs.ro_costing.initialize()
#         m.fs.zo_costing_RO_post.initialize()
#     return
#
...
#         print(f"Feed flow rate in {unit_name}: {unit_flow_rate: .3f} m3/hr")
#
#     print(f"Outlet flow rate of UF: {value(pyunits.convert(m.fs.RO_pre.UF.properties_treated[0].flow_vol, to_units=pyunits.m ** 3 / pyunits.hr)): .3f} m3/hr")
#     print(f"Flow rate of inlet of tb1: {value(pyunits.convert(m.fs.tb_pre_main.properties_in[0].flow_vol, to_units=pyunits.m ** 3 / pyunits.hr)): .3f} m3/hr")
#     print(f"Flow rate of outlet of tb1: {value(pyunits.convert(m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'], to_units=pyunits.m ** 3 / pyunits.hr)): .3f} m3/hr")
#
...
#
#     print(f"Flow rate of inlet of tb2: {value(pyunits.convert(m.fs.tb_main_post.properties_in[0].flow_vol, to_units=pyunits.m ** 3 / pyunits.hr)): .3f} m3/hr")
#     print(f"Flow rate of outlet of tb2: {value(pyunits.convert(m.fs.tb_main_post.properties_out[0].flow_vol, to_units=pyunits.m ** 3 / pyunits.hr)): .3f} m3/hr")
#
#     print(f"Feed flow rate in UV_AOP: {value(pyunits.convert(m.fs.RO_post.UV_AOP.properties_in[0].flow_vol, to_units=pyunits.m ** 3 / pyunits.hr)): .3f} m3/hr")
#     print(f"Outlet flow rate in UV_AOP: {value(pyunits.convert(m.fs.RO_post.UV_AOP.properties_treated[0].flow_vol, to_units=pyunits.m ** 3 / pyunits.hr)): .3f} m3/hr")
#
#     print(f"Flow rate (product): {value(pyunits.convert(m.fs.treated_RO.properties[0].flow_vol, to_units=pyunits.m ** 3 / pyunits.hr)): .3f} m3/hr")
#
#     # TOC removal
#     print("\nTOC concentration in each unit process")
#     print(f"TOC in feed: {value(pyunits.convert(m.fs.feed.conc_mass_comp[0, 'toc'], to_units=pyunits.mg / pyunits.L)): .3f} mg/L")
...
#             print(f"{unit_name} does not remove TOC.")
#
#     print(f"TOC after RO: {value(pyunits.convert(m.fs.tb_main_post.properties_out[0].conc_mass_comp['toc'], to_units=pyunits.mg / pyunits.L)): .3f} mg/L")
#
#     print(f"TOC before UV_AOP: {value(pyunits.convert(m.fs.RO_post.UV_AOP.properties_in[0].conc_mass_comp['toc'], to_units=pyunits.mg / pyunits.L)): .3f} mg/L")
#     print(f"TOC after UV_AOP: {value(pyunits.convert(m.fs.RO_post.UV_AOP.properties_treated[0].conc_mass_comp['toc'], to_units=pyunits.mg / pyunits.L)): .3f} mg/L")
#     toc_in_flow_mass_post = value(
#         pyunits.convert(m.fs.RO_post.UV_AOP.properties_in[0].flow_vol, to_units=pyunits.L / pyunits.s)) * value(pyunits.convert(m.fs.RO_post.UV_AOP.properties_in[0].conc_mass_comp['toc'], to_units=pyunits.mg / pyunits.L))
#     toc_out_flow_mass_post = value(
#         pyunits.convert(m.fs.RO_post.UV_AOP.properties_treated[0].flow_vol, to_units=pyunits.L / pyunits.s)) * value(pyunits.convert(m.fs.RO_post.UV_AOP.properties_treated[0].conc_mass_comp['toc'], to_units=pyunits.mg / pyunits.L))
#
#     removal_toc_post = 100 * (1 - toc_out_flow_mass_post / toc_in_flow_mass_post)
#     print(f"% removal of TOC by UV-AOP: {removal_toc_post:.2f}%")
#
#     print(
#         f"total % TOC concentration removal: {100 * (1 - value(m.fs.treated_RO.properties[0].conc_mass_comp['toc']) / value(m.fs.feed.conc_mass_comp[0, 'toc'])): .2f}%")
#
#     # Overall system recovery
...
#     print(f"System water recovery: {sys_water_recovery * 100 : .3f}%")
#     sys_tds_recovery = (
#             m.fs.treated_RO.properties[0].flow_mass_comp["tds"]()
#             / m.fs.feed.flow_mass_comp[0, "tds"]()
#     )
#     print(f"System TDS recovery: {sys_tds_recovery * 100: .3f}%")
#
#     sys_water_recoveryRO = (
#             m.fs.RO_main.RO.mixed_permeate[0].flow_mass_phase_comp["Liq", "H2O"]()
#             / m.fs.feed.flow_mass_comp[0, "H2O"]()
#     )
#     print(f"System water recovery in RO: {sys_water_recoveryRO * 100 : .3f}%")
#     sys_tds_recoveryRO = (
#             m.fs.RO_main.RO.mixed_permeate[0].flow_mass_phase_comp["Liq", "TDS"]()
#             / m.fs.feed.flow_mass_comp[0, "tds"]()
#     )
#     print(f"System TDS recovery in RO: {sys_tds_recoveryRO * 100: .3f}%")
#
#     # solute_list = m.fs.prop_zo.solute_set
#     # for solute in solute_list:
#     #     sys_recovery = (
...
#         print(f"{unit_name} capital cost: {value(pyunits.convert(u.capital_cost, to_units=pyunits.USD_2020)):.3f} $")
#         total_unit_capex_cal += value(pyunits.convert(u.capital_cost, to_units=pyunits.USD_2020))
#     for u in m.fs.zo_costing_RO_post._registered_unit_costing:
#         unit_name = u.parent_block().local_name
#         print(f"{unit_name} capital cost: {value(pyunits.convert(u.capital_cost, to_units=pyunits.USD_2020)):.3f} $")
#         total_unit_capex_cal += value(pyunits.convert(u.capital_cost, to_units=pyunits.USD_2020))
#     print(f"(Calculated) Sum of capital costs of unit processes: {total_unit_capex_cal / 1e6:.4f} M$")
...
#     # Opex
#     opex = value(pyunits.convert(m.fs.total_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year))
#     total_fixed_operating_cost = (value(pyunits.convert(m.fs.zo_costing_RO_pre.total_fixed_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year))
#                                   + value(pyunits.convert(m.fs.zo_costing_RO_post.total_fixed_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year))) # excluding RO
#     total_variable_operating_cost = (value(pyunits.convert(m.fs.zo_costing_RO_pre.total_variable_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year))
#                                      + value(pyunits.convert(m.fs.zo_costing_RO_post.total_variable_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year))) # excluding RO
#
...
#
#     # Comparison with calculated values
#     total_fixed_operating_cost_cal = (value(pyunits.convert(m.fs.zo_costing_RO_pre.aggregate_fixed_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year))
#                                       + value(pyunits.convert(m.fs.zo_costing_RO_post.aggregate_fixed_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year)))  # excluding RO
#     total_variable_operating_cost_vop_cal = (value(pyunits.convert(m.fs.zo_costing_RO_pre.aggregate_variable_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year))
#                                              + value(pyunits.convert(m.fs.zo_costing_RO_post.aggregate_variable_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year)))  # excluding RO
#     # print("Used flows:")
#     # for flow in m.fs.zo_costing_nonRO.used_flows:
#     #    print(flow)
...
#              + sum(m.fs.zo_costing_RO_post.aggregate_flow_costs[flow] for flow in m.fs.zo_costing_RO_post.used_flows))
...
#     UV_AOP_opex = value(pyunits.convert(
#         (
#                 pyunits.convert(m.fs.RO_post.UV_AOP.electricity[0] * m.fs.zo_costing_RO_post.electricity_cost,
#                                 to_units=pyunits.USD_2020 / pyunits.hour) +
#                 pyunits.convert(m.fs.RO_post.UV_AOP.chemical_flow_mass[0] * m.fs.zo_costing_RO_post.hydrogen_peroxide_cost,
#                                 to_units=pyunits.USD_2020 / pyunits.hour)
#         ) * m.fs.zo_costing_RO_post.utilization_factor,
#         to_units=pyunits.USD_2020 / pyunits.year)
#     )
#
#     Opex_dict = {"Cl": Cl_opex, "UF": UF_opex, "RO": ro_opex*1e6, "UV-AOP": UV_AOP_opex}
#     for unit, unit_opex in Opex_dict.items():
#         print(f"{unit} Opex: {unit_opex:.4f} USD/year")
#     print("---------------------------")
#
