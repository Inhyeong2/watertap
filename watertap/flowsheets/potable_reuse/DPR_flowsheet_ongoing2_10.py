...
def set_operating_conditions(m, solute_list=None, treatment_train=None):
    ...
    m.fs.feed.flow_vol[0].fix(10*0.0438)
...
def cost_output(m, treatment_train=None):
    ...
    for unit_name, unit_block in unit_blocks_zo.items():
        ...
        capex = pyunits.convert(cb.capital_cost * tif_zo, to_units=pyunits.USD_2020)  # stays with units
        ...
        mlc_cost = pyunits.convert(mlc_zo * capex / tif_zo, to_units=pyunits.USD_2020/pyunits.year)
    ...
    if treatment_train == "RBAT":
        ...
        for cb in m.fs.ro_costing._registered_unit_costing:
            capex_ro += pyunits.convert(cb.capital_cost * tif_ro, to_units=pyunits.USD_2020)
        ...
        mlc_ro_cost = mlc_ro * capex_ro / tif_ro
...
def display_cost_output(m, treatment_train=None):
    ...
    total_capex = 0 * pyunits.USD_2020
    total_opex = 0 * (pyunits.USD_2020 / pyunits.year)

    print("\n---------- Unit CAPEX & OPEX ----------")
    print(f"{'Unit':<24} {'CAPEX (MUSD_2020)':>20} {'OPEX (MUSD_2020/yr)':>24}")
    print("-" * 72)
...
def display_cost(m, treatment_train=None):
    if treatment_train == "CBAT":
        ...
        print("\n----------Unit Operating Costs (only flow cost) [non_RO / ZO]----------")
