...
def main(working_directory=None):
    ...
    add_costing_non_RO_DPR(m)
    add_costing_RO_DPR(m)
    ...
    display_results_RO_DPR(m)
    display_costing_nonRO_DPR(m)
    display_costing_RO_DPR(m)
...
def build(working_directory=None):
    ...
    m.fs.prop_zo = WaterParameterBlock(
        solute_list=[
            "cryptosporidium",
            "toc",
            "giardia_lamblia",
    ...
    m.fs.non_RO_feed = Arc(source=m.fs.feed.outlet, destination=non_RO.Ozone.inlet)
    ...
    # connections (RO DPR)
    m.fs.RO_feed = Arc(source=m.fs.feed.outlet, destination=RO_pre.Cl.inlet)
    RO_pre.s01 = Arc(source=RO_pre.Cl.inlet, destination=RO_pre.UF.inlet)
    RO_pre.s02 = Arc(source=RO_pre.UF.treated, destination=m.fs.treated_RO.inlet)
...
def set_operating_conditions(m):
    ...
    m.fs.feed.conc_mass_comp[0, "tds"].fix(feed_conc_mass_tds)
...
def initialize_system(m):
    ...
    propagate_state(m.fs.non_RO_feed)
...
def add_costing_non_RO_DPR(m):
    ...
    m.fs.specific_energy_intensity_nonRO = Expression(
    ...
    def total_capital_cost_nonRO(b):
    ...
    def total_operating_cost_nonRO(b):
        return pyunits.convert(
    ...
    def LCOT_nonRO(b):
        return (
                b.total_capital_cost_nonRO * b.zo_costing_nonRO.capital_recovery_factor
                + b.total_operating_cost_nonRO
        ) / (
                pyunits.convert(
...
def add_costing_RO_DPR(m):
    ...
    # RO DPR: cost of each unit process
    ...
    m.fs.specific_energy_intensity_RO = Expression(
        expr=(m.fs.zo_costing_RO_pre.electricity_intensity),
    ...
    def LCOT_RO(b):
        return (
                b.total_capital_cost_RO * b.zo_costing_RO_pre.capital_recovery_factor
                + b.total_operating_cost_RO
        ) / (
                pyunits.convert(
                    b.feed.properties[0].flow_vol,  # can use b.feed.flow_vol[0] as well
                    to_units=pyunits.m ** 3 / pyunits.year,
...
def display_results_nonRO_DPR(m):
    ...
    for unit in unit_names:
        ...
        toc_in_flow_mass = value(
            pyunits.convert(unit_block.properties_in[0].flow_vol, to_units=pyunits.L / pyunits.s)) * toc_in
        toc_out_flow_mass = value(
            pyunits.convert(unit_block.properties_treated[0].flow_vol, to_units=pyunits.L / pyunits.s)) * toc_out
        ...
        if "toc" in unit_block.config.database.get_unit_operation_parameters(unit_block._tech_type)[
            ...
            print(f"% removal of TOC by {unit}: {removal_toc:.2f}%") # based on flow_mass_comp (not conc_mass_comp)
        else:
    ...
    for unit in unit_names:
        ...
        crypto_out = value(pyunits.convert(unit_block.properties_treated[0].conc_mass_comp["cryptosporidium"],
                                           to_units=pyunits.mg / pyunits.L))
        # print(f"Cryptosporidium before {unit}: {crypto_in: .8f} mg/L")
        # print(f"Cryptosporidium after {unit}: {crypto_out: .8f} mg/L")

        crypto_in_flow_mass = value(
            pyunits.convert(unit_block.properties_in[0].flow_vol, to_units=pyunits.L / pyunits.s)) * crypto_in
        crypto_out_flow_mass = value(
            pyunits.convert(unit_block.properties_treated[0].flow_vol, to_units=pyunits.L / pyunits.s)) * crypto_out
        ...
        if "cryptosporidium" in unit_block.config.database.get_unit_operation_parameters(unit_block._tech_type)[
            ...
            pass
    print(
        f"Total log removal ratio of Cryptosporidium (concentration ratio): "
        f"{-math.log10(value(m.fs.treated_nonRO.properties[0].conc_mass_comp['cryptosporidium']) / value(m.fs.feed.conc_mass_comp[0, 'cryptosporidium'])): .2f} log"
    )
...
def display_results_RO_DPR(m):
    ...
    unit_names = ["Cl", "UF"]
    for unit in unit_names:
        getattr(m.fs.RO_pre, unit).report()

    # Flow rate
    print("\nFeed flow rate in each unit process")
    print(f"Feed flow rate: {value(pyunits.convert(m.fs.feed.flow_vol[0], to_units=pyunits.m**3 / pyunits.hr,)): .3f} m3/hr")
    for unit in unit_names:
        unit_flow_rate = value(pyunits.convert(getattr(m.fs.RO_pre, unit).properties_in[0].flow_vol, to_units=pyunits.m ** 3 / pyunits.hr))
        print(f"Feed flow rate in {unit}: {unit_flow_rate: .3f} m3/hr")
    ...
    print(f"TOC in feed: {value(pyunits.convert(m.fs.feed.conc_mass_comp[0, 'toc'], to_units=pyunits.mg / pyunits.L)): .3f} mg/L")
    for unit in unit_names:
        unit_block = getattr(m.fs.RO_pre, unit)
        toc_in = value(
            pyunits.convert(unit_block.properties_in[0].conc_mass_comp["toc"], to_units=pyunits.mg / pyunits.L))
        toc_out = value(
            pyunits.convert(unit_block.properties_treated[0].conc_mass_comp["toc"], to_units=pyunits.mg / pyunits.L))
    ...
    for unit in unit_names:
        ...
        print(f"TSS after {unit}: {tss_out: .3f} mg/L")
    print(
        f"total % TSS concentration removal: {100 * (1 - value(m.fs.treated_RO.properties[0].conc_mass_comp['tss']) / value(m.fs.feed.conc_mass_comp[0, 'tss'])): .2f}%")
    ...
    print(
        f"Total log removal ratio of Cryptosporidium (concentration ratio): "
        f"{-math.log10(value(m.fs.treated_RO.properties[0].conc_mass_comp['cryptosporidium']) / value(m.fs.feed.conc_mass_comp[0, 'cryptosporidium'])): .2f} log"
    )
...
def display_costing_nonRO_DPR(m):
    # Capex
    print("\n----------Costing----------\n")
    capex = value(pyunits.convert(m.fs.total_capital_cost_nonRO, to_units=pyunits.MUSD_2020))
    ...
    # Opex
    opex = value(pyunits.convert(m.fs.total_operating_cost_nonRO, to_units=pyunits.MUSD_2020 / pyunits.year))
    ...
    total_flow_cost_cal = value(
        pyunits.convert(
            (
                    m.fs.zo_costing_nonRO.aggregate_flow_costs["electricity"] +
                    m.fs.zo_costing_nonRO.aggregate_flow_costs["activated_carbon"] +
                    m.fs.zo_costing_nonRO.aggregate_flow_costs["chlorine"] +
                    m.fs.zo_costing_nonRO.aggregate_flow_costs["hydrogen_peroxide"]
    ...
    capex_norm = (
        value(pyunits.convert(m.fs.total_capital_cost_nonRO, to_units=pyunits.USD_2020))
        / feed_flowrate
    )

    annual_investment = value(
        pyunits.convert(
            m.fs.total_capital_cost_nonRO * m.fs.zo_costing_nonRO.capital_recovery_factor
            + m.fs.total_operating_cost_nonRO,
            to_units=pyunits.USD_2020 / pyunits.year,
        )
    )
    ...
    opex_fraction = (
        ...
        * value(
            pyunits.convert(
                m.fs.total_operating_cost_nonRO, to_units=pyunits.USD_2020 / pyunits.year
            )
        )
        / annual_investment
    )
    ...
    lcot = value(pyunits.convert(m.fs.LCOT_nonRO, to_units=pyunits.USD_2020 / pyunits.m ** 3))
    lcow = value(pyunits.convert(m.fs.LCOW_nonRO, to_units=pyunits.USD_2020 / pyunits.m ** 3))
    sec = m.fs.specific_energy_intensity_nonRO()

    #opex = value(pyunits.convert(m.fs.zo_costing_nonRO.total_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year))
...
def display_costing_RO_DPR(m):
    # Capex
    print("\n----------Costing----------\n")
    capex = value(pyunits.convert(m.fs.total_capital_cost_RO, to_units=pyunits.MUSD_2020))
    print(f"Total Capital Cost: {capex:.4f} M$")
    ...
    # Opex
    opex = value(pyunits.convert(m.fs.total_operating_cost_RO, to_units=pyunits.MUSD_2020 / pyunits.year))
    ...
    Opex_dict = {"Cl": Cl_opex, "UF": UF_opex}
    for unit, unit_opex in Opex_dict.items():
        print(f"{unit} Opex: {unit_opex:.4f} USD/year")
    print("---------------------------\n")
    ...
    capex_norm = (
        value(pyunits.convert(m.fs.total_capital_cost_RO, to_units=pyunits.USD_2020))
        / feed_flowrate
    )

    annual_investment = value(
        pyunits.convert(
            m.fs.total_capital_cost_RO * m.fs.zo_costing_RO_pre.capital_recovery_factor
            + m.fs.total_operating_cost_RO,
            to_units=pyunits.USD_2020 / pyunits.year,
        )
    )
    ...
    opex_fraction = (
        ...
        * value(
            pyunits.convert(
                m.fs.total_operating_cost_RO, to_units=pyunits.USD_2020 / pyunits.year
            )
        )
        / annual_investment
    )
    ...
    #annual_water_production = value(pyunits.convert( m.fs.annual_water_production, to_units=pyunits.m ** 3 / pyunits.year))
    lcot = value(pyunits.convert(m.fs.LCOT_RO, to_units=pyunits.USD_2020 / pyunits.m ** 3))
    lcow = value(pyunits.convert(m.fs.LCOW_RO, to_units=pyunits.USD_2020 / pyunits.m ** 3))
    sec = m.fs.specific_energy_intensity_RO()
