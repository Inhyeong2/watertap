...
from watertap.flowsheets.nonRO_DPR.imaginary_mixer_zo import ImaginaryMixerZO
...
def build_RO(working_directory=None, solute_list=None): # TODO: [defalut] solute_list is identical to that in build_nonRO
    ...
    RO_mix.img_mix = ImaginaryMixerZO(property_package=m.fs.prop_zo, database=m.db)
    ...
    m.fs.s_sep_mix = Arc(source=RO_sep.img_sep.byproduct, destination=RO_mix.img_mix.inlet1)
    ...
    m.fs.s_tb2_mix = Arc(source=m.fs.tb_main_post.outlet, destination=RO_mix.img_mix.inlet2)


    m.fs.s_mix_prod = Arc(source=RO_mix.img_mix.treated, destination=m.fs.treated_RO.inlet)
...
def set_operating_conditions(model, is_ro=False):
    ...
    if not is_ro:  # non-RO DPR
        ...
        RO_mix.img_mix.load_parameters_from_database(use_default_removal=True)
...
def initialize_system(model, is_ro=False, solute_list=None):
    ...
    if not is_ro:  # non-RO DPR
        ...
        # Propagate state from imaginary separator (inlet1) and tb_main_post (inlet2) to imaginary mixer
        propagate_state(model.fs.s_sep_mix)
        propagate_state(model.fs.s_tb2_mix)
        RO_mix.img_mix.initialize()

        # Propagate state from mixer to treated_RO
        propagate_state(model.fs.s_mix_prod)
...
def display_results_RO_DPR(m):
    ...
    # Flow rate
    print("\nVolumetric flowrate in each unit process")
    for unit_name, unit_block in unit_models.items():
        unit_flow_rate_in = value(
            pyunits.convert(unit_block.properties_in[0].flow_mass_comp["H2O"], to_units=pyunits.kg / pyunits.hr))
        unit_flow_rate_out = value(
            pyunits.convert(unit_block.properties_treated[0].flow_mass_comp["H2O"], to_units=pyunits.kg / pyunits.hr))
        print(f"Inlet flow rate in {unit_name}: {unit_flow_rate_in: .3f} kg/hr")
        print(f"Outlet flow rate in {unit_name}: {unit_flow_rate_in: .3f} kg/hr")

    print(f"Inlet flow rate in img_sep: {value(pyunits.convert(m.fs.RO_sep.img_sep.properties_in[0].flow_mass_comp['H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")
    print(f"Outlet flow rate of img_sep (to tb1): {value(pyunits.convert(m.fs.RO_sep.img_sep.properties_treated[0].flow_mass_comp['H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")

    print(
        f"Flow rate of inlet1 (img_sep) to img_mix: {value(pyunits.convert(m.fs.RO_sep.img_sep.properties_byproduct[0].flow_mass_comp['H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")

    print(f"Flow rate of inlet of tb1: {value(pyunits.convert(m.fs.tb_pre_main.properties_in[0].flow_mass_comp['H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")
    print(f"Flow rate of outlet of tb1: {value(pyunits.convert(m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp['Liq','H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")

    print(f"Feed flow rate in RO: {value(pyunits.convert(m.fs.RO_main.RO.feed_side.properties[0, 0].flow_mass_phase_comp['Liq','H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")
    print(f"Flow rate of RO permeate: {value(pyunits.convert(m.fs.RO_main.RO.mixed_permeate[0].flow_mass_phase_comp['Liq','H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")

    print(f"Flow rate of inlet of tb2: {value(pyunits.convert(m.fs.tb_main_post.properties_in[0].flow_mass_phase_comp['Liq','H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")
    print(f"Flow rate of outlet of tb2: {value(pyunits.convert(m.fs.tb_main_post.properties_out[0].flow_mass_comp['H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")

    print(f"Flow rate of inlet1 (img_sep) of img_mix: {value(pyunits.convert(m.fs.RO_mix.img_mix.properties_in1[0].flow_mass_comp['H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")
    print(f"Flow rate of input2 (tb2) of img_mixer: {value(pyunits.convert(m.fs.RO_mix.img_mix.properties_in2[0].flow_mass_comp['H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")
    print(f"Flow rate after img_mixer: {value(pyunits.convert(m.fs.RO_mix.img_mix.properties_treated[0].flow_mass_comp['H2O'], to_units=pyunits.kg / pyunits.hr)): .3f} kg/hr")
