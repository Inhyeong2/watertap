...
def optimize_operation(m, state = None, effluent_type = None, treatment_train = None):
    if treatment_train == "CBAT": #non-RO
        ...
        #todo should be classified into two different group: RBAT and CBAT
...
def display_results(m, treatment_train=None):
    if treatment_train == "CBAT":
        ...
        for t in m.fs.time:
            lhs = pyo.value(m.fs.non_RO.residual_O3[t].body)
            rhs = pyo.value(m.fs.non_RO.residual_O3[t].lower)
            print(f"Residual O3 constraint: {lhs:.4f} >= {rhs:.4f}?", "✅" if lhs >= rhs else "❌")
