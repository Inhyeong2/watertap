...
def main(state="FL", treatment_train="RBAT", effluent_type="TERTIARY"):
    ...
    if treatment_train == "RBAT":
        adaptive_tds_sweep(m, solute_list=solute_list, target_tds=value(solute_list["tds"]))
...
def set_operating_conditions(m, flow_vol=10 * 0.0438, solute_list=None, treatment_train=None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR
        ...
        #
        # # calculate properties_out state of translator block
        # m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp[
        #     ...].unfix()  # Unfix the mass flow rates to recalculate them based on the new conditions
        # m.fs.tb_pre_main.properties_out[0].calculate_state(
        #     var_args={
        #         (
        #             "conc_mass_phase_comp",
        #             ("Liq", "TDS"),
        #         ): 15,  # feed mass concentration
        #         ("flow_vol_phase", "Liq"): flow_vol_to_RO,
...
def adaptive_tds_sweep(m, solute_list=None, target_tds=None, initial_tds=15, step_sizes=[5, 2.5, 1, 0.5, 0.25, 0.1]):
    print("###### Adaptive TDS Sweep #######")
    current_tds = initial_tds
    step_idx = 0

    while True:
        print(f"\n##### Trying TDS: {current_tds} g/L (step: {step_sizes[step_idx]} g/L) #####")
        try:
            ...
            # If target TDS is reached, we're done
            if current_tds == target_tds:
                print(f"✅ Successfully reached the target TDS: {target_tds} g/L")
                break

            # Prepare next TDS attempt
            tentative_tds = current_tds - step_sizes[step_idx]
            if tentative_tds < target_tds:
                if step_idx < len(step_sizes) - 1:
                    step_idx += 1
                    print(f"Next step is too large. Reducing step size to {step_sizes[step_idx]}")
                else:
                    print(f"Trying final TDS at exact target: {target_tds} g/L")
                    current_tds = target_tds
            else:
                current_tds = tentative_tds

        except Exception as e:
            print(f"❌ Normal solve failed at TDS {current_tds} g/L: {str(e)}")

            # Try fallback overpressure guesses
            success = False
            for guess in [60, 50, 20, 15, 10, 8, 6, 4]:
                ...
                try:
                    ...
                    print(f"✅ Recovered at TDS {current_tds} with overpressure guess {guess}")
                    success = True
                    break
                except Exception as e2:
                    print(f"⚠️ Overpressure guess {guess} failed: {str(e2)}")

            if success:
                if current_tds == target_tds:
                    print(f"✅ Successfully reached the target TDS: {target_tds} g/L (with overpressure guess)")
                    break
                # Proceed to next TDS without retaining overpressure
                tentative_tds = current_tds - step_sizes[step_idx]
                if tentative_tds < target_tds:
                    if step_idx < len(step_sizes) - 1:
                        step_idx += 1
            ...
            # All overpressure guesses failed → reduce step size
            step_idx += 1
            if step_idx >= len(step_sizes):
                print("❌ All step sizes and pressure guesses exhausted. Stopping loop.")
                break
            else:
    ...
    if current_tds > target_tds:
        raise RuntimeError(
            f"\n⚠️ Could not reach the minimum target TDS ({target_tds} g/L). "
            f"Final TDS tested: {current_tds} g/L"
        )
...
def solve_RO_initial_conditions(m,
    ...
    if overpressure_sweep == False:
        ...
        over_pressure_estimate = 10 ** (
                    -math.log10(m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value) - 2.5)
