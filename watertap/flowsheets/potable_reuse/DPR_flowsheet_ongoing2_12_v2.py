...
def build_RO(working_directory="local", state=None, solute_list=None, effluent_type=None, treatment_train=None, LRVO3_req=None, LRVClvirus_req=None, LRVClgiardia_req=None):
    ...
    RO_main.pump1 = Pump(property_package=m.fs.prop_ro)
    RO_main.RO1 = ReverseOsmosis0D(
    ...
    RO_main.pump2 = Pump(property_package=m.fs.prop_ro)
    RO_main.RO2 = ReverseOsmosis0D(
        property_package=m.fs.prop_ro,
        has_pressure_change=True,
        pressure_change_type=PressureChangeType.calculated,
        mass_transfer_coefficient=MassTransferCoefficient.calculated,
    ...
    m.fs.s_tb1_RO = Arc(source=m.fs.tb_pre_main.outlet, destination=RO_main.pump1.inlet)
    RO_main.s01 = Arc(source=RO_main.pump1.outlet, destination=RO_main.RO1.inlet)
    RO_main.s02 = Arc(source=RO_main.RO1.retentate, destination=RO_main.pump2.inlet)
    RO_main.s03 = Arc(source=RO_main.RO1.permeate, destination=RO_main.mixer.RO1)
    RO_main.s04 = Arc(source=RO_main.pump2.outlet, destination=RO_main.RO2.inlet)
    RO_main.s05 = Arc(source=RO_main.RO2.retentate, destination=RO_main.ERD.inlet)
    RO_main.s06 = Arc(source=RO_main.RO2.permeate, destination=RO_main.mixer.RO2)

    m.fs.s_disposal  = Arc(source=RO_main.ERD.outlet, destination=m.fs.brine.inlet)
    m.fs.s_RO_tb2 = Arc(source=RO_main.mixer.outlet, destination=m.fs.tb_main_post.inlet)
...
def set_operating_conditions(m, solute_list=None, treatment_train=None):
    ...
    m.fs.feed.flow_vol[0].fix(5*0.0438126)
    ...
    if treatment_train == "CBAT":  # non-RO DPR
        ...
        # Pump1
        RO_main.pump1.efficiency_pump.fix(0.80)
        RO_main.pump1.outlet.pressure[0].fix(40*101325) # this operating pressure is good initial guess for low TDS condition (0.5-1 kg/m3 of TDS)

        # RO1 unit
        RO_main.RO1.A_comp.fix(4.2e-12)  # membrane water permeability
        RO_main.RO1.B_comp.fix(3.5e-8)  # membrane salt permeability
        RO_main.RO1.feed_side.channel_height.fix(1e-3)  # channel height in membrane stage [m]
        RO_main.RO1.length.fix(8)  # membrane length [m]
        RO_main.RO1.feed_side.spacer_porosity.fix(0.85)  # spacer porosity in membrane stage [-]
        RO_main.RO1.permeate.pressure[0].fix(101325)  # atmospheric pressure [Pa]
        RO_main.RO1.feed_side.velocity[0, 0].fix(0.25)

        feed_side_area_guess = m.fs.feed.flow_vol[0].value / RO_main.RO1.feed_side.velocity[0, 0].value
        width_guess = feed_side_area_guess / RO_main.RO1.feed_side.spacer_porosity.value / RO_main.RO1.feed_side.channel_height.value

        RO_main.RO1.feed_side.area.setub(None)
        RO_main.RO1.width.setub(None)
        RO_main.RO1.area.setub(None)
        RO_main.RO1.width.fix(width_guess)
        RO_main.RO1.width.unfix()  # Unfixed variables are width and recovery
        RO_main.RO1.recovery_vol_phase[0, "Liq"].fix(0.5)
        RO_main.RO1.recovery_vol_phase[0, "Liq"].unfix()

        # Pump2
        RO_main.pump2.efficiency_pump.fix(0.80)
        RO_main.pump2.outlet.pressure[0].fix(50 * 101325)

        # RO2 unit
        RO_main.RO2.A_comp.fix(4.2e-12)  # membrane water permeability
        RO_main.RO2.B_comp.fix(3.5e-8)  # membrane salt permeability
        RO_main.RO2.feed_side.channel_height.fix(1e-3)  # channel height in membrane stage [m]
        RO_main.RO2.length.fix(8)  # membrane length [m]
        RO_main.RO2.feed_side.spacer_porosity.fix(0.85)  # spacer porosity in membrane stage [-]
        RO_main.RO2.permeate.pressure[0].fix(101325)  # atmospheric pressure [Pa]
        RO_main.RO2.feed_side.velocity[0, 0].fix(0.25)

        feed_side_area_guess2 = (1-RO_main.RO1.recovery_vol_phase[0, "Liq"].value) * m.fs.feed.flow_vol[0].value / RO_main.RO2.feed_side.velocity[0, 0].value
        width_guess2 = feed_side_area_guess2 / RO_main.RO2.feed_side.spacer_porosity.value / RO_main.RO2.feed_side.channel_height.value

        RO_main.RO2.feed_side.area.setub(None)
        RO_main.RO2.width.setub(None)
        RO_main.RO2.area.setub(None)
        RO_main.RO2.width.fix(width_guess2)
        RO_main.RO2.width.unfix()  # Unfixed variables are width and recovery
        RO_main.RO2.recovery_vol_phase[0, "Liq"].fix(0.5)
        RO_main.RO2.recovery_vol_phase[0, "Liq"].unfix()
...
def scale_system(m, treatment_train = None):
    ...
    if treatment_train == "CBAT":
        ...
        #### Main pump1 ####
        op_pressure = RO_main.pump1.control_volume.properties_out[0].pressure.value
        iscale.set_scaling_factor(RO_main.pump1.control_volume.work, 1 / (
                op_pressure * m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value /
                RO_main.pump1.efficiency_pump[0].value))
        iscale.set_scaling_factor(RO_main.pump1.work_fluid[0], 1 / (
                op_pressure * m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value))
        iscale.set_scaling_factor(RO_main.pump1.control_volume.properties_out[0].pressure, 1 / op_pressure)
        iscale.set_scaling_factor(RO_main.pump1.control_volume.properties_in[0].pressure, 1 / 101325)
        iscale.set_scaling_factor(
            RO_main.pump1.control_volume.properties_out[0].flow_vol_phase["Liq"],
            1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value
        )

        #### RO unit1 ####
        recovery = RO_main.RO1.recovery_vol_phase[0, "Liq"].value
        # === Geometric parameters of RO units ===
        iscale.set_scaling_factor(RO_main.RO1.length, 1e-1)
        iscale.set_scaling_factor(RO_main.RO1.feed_side.channel_height, 1e3)
        iscale.set_variable_scaling_from_current_value(RO_main.RO1.width)
        calculate_variable_from_constraint(RO_main.RO1.area, RO_main.RO1.eq_area)
        iscale.set_variable_scaling_from_current_value(RO_main.RO1.area)
        calculate_variable_from_constraint(
            RO_main.RO1.feed_side.area, RO_main.RO1.feed_side.eq_area
        )
        iscale.set_variable_scaling_from_current_value(RO_main.RO1.feed_side.area)
        iscale.set_scaling_factor(RO_main.RO1.feed_side.spacer_porosity, 1)
        iscale.set_scaling_factor(RO_main.RO1.A_comp[0, "H2O"], 1e12)
        iscale.set_scaling_factor(RO_main.RO1.B_comp[0, "TDS"], 1e8)
        iscale.set_scaling_factor(RO_main.RO1.mixed_permeate[0].pressure, 1e-5)
        iscale.set_scaling_factor(RO_main.RO1.recovery_vol_phase[0, "Liq"], 2)

        # === Operating conditions ===
        hc = RO_main.RO1.feed_side.channel_height.value  # channel height [m]
        width = RO_main.RO1.width.value  # width of the module [m]
        length = RO_main.RO1.length.value  # length of the module [m]
        q_feed = m.fs.tb_pre_main.properties_out[0].flow_vol_phase["Liq"].value  # feed flow rate [m³/s]
        porosity = RO_main.RO1.feed_side.spacer_porosity.value  # dimensionless
        ...
        for v in RO_main.RO1.feed_side.N_Sc_comp.values():
            iscale.set_scaling_factor(v, 1 / sc_in)
        for v in RO_main.RO1.feed_side.N_Re.values():
            iscale.set_scaling_factor(v, 1 / re_avg)
        for v in RO_main.RO1.feed_side.friction_factor_darcy.values():
            iscale.set_scaling_factor(v, 1 / f_avg)
        for v in RO_main.RO1.feed_side.N_Sh_comp.values():
            sh_avg = re_avg ** 0.36 * sc_avg ** 0.36
            iscale.set_scaling_factor(v, 1 / sh_avg)

        for v in RO_main.RO1.feed_side.dP_dx.values():
            iscale.set_scaling_factor(v, length / dp_avg)

        iscale.set_scaling_factor(RO_main.RO1.deltaP, 1 / dp_avg)

        # Feed side (bulk)
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_in[0].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value
                                  )
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_out[0].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value / (
                                              1 - recovery)
                                  )
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_in[0].flow_mass_phase_comp["Liq", "TDS"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_out[0].flow_mass_phase_comp["Liq", "TDS"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )

        # Feed side (interface)
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_interface[0, 0].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value
                                  )
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_interface[0, 1].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value / (
                                              1 - recovery)
                                  )
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_interface[0, 0].flow_mass_phase_comp["Liq", "TDS"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_interface[0, 1].flow_mass_phase_comp["Liq", "TDS"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )

        # Permeate side
        iscale.set_scaling_factor(RO_main.RO1.permeate_side[0, 0].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp[
                                      "Liq", "H2O"].value / recovery
                                  )
        iscale.set_scaling_factor(RO_main.RO1.permeate_side[0, 1].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp[
                                      "Liq", "H2O"].value / recovery
                                  )

        iscale.set_scaling_factor(RO_main.RO1.permeate_side[0, 0].flow_mass_phase_comp["Liq", "TDS"],
                                  100 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )
        iscale.set_scaling_factor(RO_main.RO1.permeate_side[0, 1].flow_mass_phase_comp["Liq", "TDS"],
                                  100 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )

        iscale.set_scaling_factor(
            RO_main.RO1.mixed_permeate[0].flow_mass_phase_comp["Liq", "H2O"],
            1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value / recovery
        )
        iscale.set_scaling_factor(
            RO_main.RO1.mixed_permeate[0].flow_mass_phase_comp["Liq", "TDS"],
            100 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
        )

        iscale.set_scaling_factor(RO_main.RO1.mass_transfer_phase_comp[0, "Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp[
                                      "Liq", "H2O"].value / recovery)
        iscale.set_scaling_factor(RO_main.RO1.mass_transfer_phase_comp[0, "Liq", "TDS"],
                                  100 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value)

        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_in[0].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value)
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_out[0].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value / (1 - recovery))
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_interface[0, 0].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value)
        iscale.set_scaling_factor(RO_main.RO1.feed_side.properties_interface[0, 1].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value / (1 - recovery))
        iscale.set_scaling_factor(RO_main.RO1.permeate_side[0, 0].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value / recovery)
        iscale.set_scaling_factor(RO_main.RO1.permeate_side[0, 0].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value / recovery)

        iscale.calculate_scaling_factors(RO_main.RO1)

        #### Main pump2 ####
        op_pressure2 = RO_main.pump2.control_volume.properties_out[0].pressure.value
        iscale.set_scaling_factor(RO_main.pump2.control_volume.work, 1 / (
                op_pressure2 * m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value * (1-recovery)/
                RO_main.pump2.efficiency_pump[0].value))
        iscale.set_scaling_factor(RO_main.pump2.work_fluid[0], 1 / (
                op_pressure2 * m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value) * (1-recovery))
        iscale.set_scaling_factor(RO_main.pump2.control_volume.properties_out[0].pressure, 1 / op_pressure2)
        iscale.set_scaling_factor(RO_main.pump2.control_volume.properties_in[0].pressure, 1 / op_pressure)
        iscale.set_scaling_factor(
            RO_main.pump2.control_volume.properties_out[0].flow_vol_phase["Liq"],
            1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value / (1-recovery)
        )

        #### RO unit2 ####
        recovery2 = RO_main.RO2.recovery_vol_phase[0, "Liq"].value
        # === Geometric parameters of RO units ===
        iscale.set_scaling_factor(RO_main.RO2.length, 1e-1)
        iscale.set_scaling_factor(RO_main.RO2.feed_side.channel_height, 1e3)
        iscale.set_variable_scaling_from_current_value(RO_main.RO2.width)
        calculate_variable_from_constraint(RO_main.RO2.area, RO_main.RO2.eq_area)
        iscale.set_variable_scaling_from_current_value(RO_main.RO2.area)
        calculate_variable_from_constraint(
            RO_main.RO2.feed_side.area, RO_main.RO2.feed_side.eq_area
        )
        iscale.set_variable_scaling_from_current_value(RO_main.RO2.feed_side.area)
        iscale.set_scaling_factor(RO_main.RO2.feed_side.spacer_porosity, 1)
        iscale.set_scaling_factor(RO_main.RO2.A_comp[0, "H2O"], 1e12)
        iscale.set_scaling_factor(RO_main.RO2.B_comp[0, "TDS"], 1e8)
        iscale.set_scaling_factor(RO_main.RO2.mixed_permeate[0].pressure, 1e-5)
        iscale.set_scaling_factor(RO_main.RO2.recovery_vol_phase[0, "Liq"], 2)

        # === Operating conditions ===
        hc2 = RO_main.RO2.feed_side.channel_height.value  # channel height [m]
        width2 = RO_main.RO2.width.value  # width of the module [m]
        length2 = RO_main.RO2.length.value  # length of the module [m]
        q_feed2 = m.fs.tb_pre_main.properties_out[0].flow_vol_phase["Liq"].value * (1-recovery)  # feed flow rate [m³/s]
        porosity2 = RO_main.RO2.feed_side.spacer_porosity.value  # dimensionless
        dh2 = 4 * porosity2 / (2 / hc2 + (1 - porosity2) * 8 / hc2)

        # Inlet and outlet velocities
        v_inlet2 = q_feed2 / (width2 * hc2)  # inlet velocity [m/s]
        v_exit2 = v_inlet2 * (1 - recovery2)  # exit velocity [m/s]
        ...
        # === Inlet calculations ===
        re_in2 = dens * v_inlet2 * 2 * dh2 / visc
        sc_in2 = visc / (dens * diff)
        f_in2 = 0.42 + (189.3 / re_in2)

        # === Exit calculations ===
        re_out2 = dens * v_exit2 * 2 * dh2 / visc
        sc_out2 = sc_in2  # usually same properties
        f_out2 = 0.42 + (189.3 / re_out2)

        # === Average values ===
        v_avg2 = 0.5 * (v_inlet2 + v_exit2)
        re_avg2 = 0.5 * (re_in2 + re_out2)
        f_avg2 = 0.5 * (f_in2 + f_out2)
        sc_avg2 = 0.5 * (sc_in2 + sc_out2)

        # === Pressure drop using Darcy-Weisbach (with averaged values) ===
        dp_in2 = -dens * v_inlet2 ** 2 / 2 * f_in2 / dh2  # [Pa/m]
        dp_out2 = -dens * v_exit2 ** 2 / 2 * f_out2 / dh2  # [Pa/m]
        dp_avg2 = 0.5 * (dp_in2 + dp_out2)  # average pressure drop [Pa/m]

        # Set scaling for dimensionless numbers based on representative (inlet) values
        for v in RO_main.RO2.feed_side.N_Sc_comp.values():
            iscale.set_scaling_factor(v, 1 / sc_in2)
        for v in RO_main.RO2.feed_side.N_Re.values():
            iscale.set_scaling_factor(v, 1 / re_avg2)
        for v in RO_main.RO2.feed_side.friction_factor_darcy.values():
            iscale.set_scaling_factor(v, 1 / f_avg2)
        for v in RO_main.RO2.feed_side.N_Sh_comp.values():
            sh_avg2 = re_avg2 ** 0.36 * sc_avg2 ** 0.36
            iscale.set_scaling_factor(v, 1 / sh_avg2)

        for v in RO_main.RO2.feed_side.dP_dx.values():
            iscale.set_scaling_factor(v, length2 / dp_avg2)

        iscale.set_scaling_factor(RO_main.RO2.deltaP, 1 / dp_avg2)

        # Feed side (bulk)
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_in[0].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value / (1-recovery)
                                  )
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_out[0].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value / (1-recovery) / (
                                          1 - recovery2)
                                  )
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_in[0].flow_mass_phase_comp["Liq", "TDS"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_out[0].flow_mass_phase_comp["Liq", "TDS"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )

        # Feed side (interface)
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_interface[0, 0].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value / (1-recovery)
                                  )
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_interface[0, 1].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value / (1-recovery) / (
                                          1 - recovery2)
                                  )
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_interface[0, 0].flow_mass_phase_comp["Liq", "TDS"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_interface[0, 1].flow_mass_phase_comp["Liq", "TDS"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )

        # Permeate side
        iscale.set_scaling_factor(RO_main.RO2.permeate_side[0, 0].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp[
                                      "Liq", "H2O"].value / (1-recovery) / recovery2
                                  )
        iscale.set_scaling_factor(RO_main.RO2.permeate_side[0, 1].flow_mass_phase_comp["Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp[
                                      "Liq", "H2O"].value / (1-recovery) / recovery2
                                  )

        iscale.set_scaling_factor(RO_main.RO2.permeate_side[0, 0].flow_mass_phase_comp["Liq", "TDS"],
                                  100 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )
        iscale.set_scaling_factor(RO_main.RO2.permeate_side[0, 1].flow_mass_phase_comp["Liq", "TDS"],
                                  100 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
                                  )

        iscale.set_scaling_factor(
            RO_main.RO2.mixed_permeate[0].flow_mass_phase_comp["Liq", "H2O"],
            1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value / (1-recovery) / recovery2
        )
        iscale.set_scaling_factor(
            RO_main.RO2.mixed_permeate[0].flow_mass_phase_comp["Liq", "TDS"],
            100 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value
        )

        iscale.set_scaling_factor(RO_main.RO2.mass_transfer_phase_comp[0, "Liq", "H2O"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp[
                                      "Liq", "H2O"].value / (1-recovery) / recovery2)
        iscale.set_scaling_factor(RO_main.RO2.mass_transfer_phase_comp[0, "Liq", "TDS"],
                                  100 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "TDS"].value)

        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_in[0].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value/ (1-recovery))
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_out[0].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value / (1-recovery) / (1 - recovery2))
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_interface[0, 0].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value/ (1-recovery))
        iscale.set_scaling_factor(RO_main.RO2.feed_side.properties_interface[0, 1].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value / (1-recovery) / (1 - recovery2))
        iscale.set_scaling_factor(RO_main.RO2.permeate_side[0, 0].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value / (1-recovery)/ recovery2)
        iscale.set_scaling_factor(RO_main.RO2.permeate_side[0, 0].flow_vol_phase["Liq"],
                                  1 / m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value / (1-recovery)/ recovery2)

        iscale.calculate_scaling_factors(RO_main.RO2)


        # ERD unit
        iscale.set_scaling_factor(
            RO_main.ERD.control_volume.properties_in[0].flow_mass_phase_comp["Liq", "H2O"],
            1 / m.fs.tb_pre_main.properties_out[0].flow_mass_phase_comp["Liq", "H2O"].value / (1-recovery) / (1 - recovery2)
        )
        iscale.set_scaling_factor(
        ...
        iscale.set_scaling_factor(RO_main.ERD.control_volume.work, 1 / (
                op_pressure2 * m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value * (1-recovery) * (1 - recovery2) /
                RO_main.ERD.efficiency_pump[0].value))
        iscale.set_scaling_factor(RO_main.pump.work_fluid[0], 1 / (
                op_pressure2 * m.fs.tb_pre_main.properties_out[0].flow_vol_phase['Liq'].value * (1 - recovery) * (1 - recovery2) )
                                  )
        iscale.set_scaling_factor(RO_main.ERD.control_volume.properties_in[0].pressure, 1 / op_pressure2)
...
def initialize_system(m, treatment_train = None):
    ...
    if treatment_train == "CBAT":  # non-RO DPR
        ...
        RO_main.pump1.initialize()
        propagate_state(RO_main.s01)

        RO_main.RO1.initialize()
        propagate_state(RO_main.s02)

        RO_main.pump2.initialize()
        propagate_state(RO_main.s04)

        RO_main.RO2.initialize()
        propagate_state(RO_main.s05)

        RO_main.ERD.initialize()
        propagate_state(m.fs.s_disposal)
        m.fs.brine.initialize()

        RO_main.mixer.initialize()
        propagate_state(RO_main.s03)
        propagate_state(RO_main.s06)
