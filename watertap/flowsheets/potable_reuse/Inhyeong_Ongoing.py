...
def main(working_directory=None):
    ...
    LRV_req, details, state, treatment_train, effluent_type, solute_list, system_capacity = DPR_initial_setting(
        state="CO", treatment_train="CBAT", effluent_type="TERTIARY", system_capacity=(50 * pyunits.Mgallons / pyunits.day)
    )
...
#     sweep_params['System Capacity (MGD)'] = LinearSample(m.fs.RO.recovery_mass_phase_comp[0, 'Liq', 'H2O'], 0.3, 0.7, 4)
...
#     sweep_params['Membrane area-fixed-state'] = PredeterminedFixedSample(m.fs.RO.area, [True, False])
...
def build_nonRO(working_directory=None, state=None, solute_list=None, effluent_type=None, system_capacity=None, LRVO3_req=None, LRVClvirus_req=None, LRVClgiardia_req=None):
    # flowsheet set up
    m = ConcreteModel()
...
def optimize_operation(m, state = None, effluent_type = None, treatment_train = None):
    if treatment_train == "CBAT": #non-RO
        ...
        if state == "CA":
            ...
            if hasattr(non_RO, "LRVO3req"):
                non_RO.del_component("LRVO3req")

            non_RO.LRVO3req = Constraint(
                m.fs.time,
        ...
        # if non_RO.Cl.config.LRVCl_required_for_virus != 0 or non_RO.Cl.config.LRVCl_required_for_giardia != 0:
        #     # Constraint: Residual Cl conc
        #     if hasattr(non_RO, "residual_Cl"):
        #         non_RO.del_component("residual_Cl")
        #     non_RO.residual_Cl = Constraint(
        #         m.fs.time,
        #         rule=lambda b, t: (non_RO.Cl.chlorine_dose[t] + non_RO.Cl.initial_chlorine_demand[t]) * pyo.exp(
        #             pyunits.convert(
        #                 -non_RO.Cl.chlorine_decay_rate[t] * non_RO.Cl.contact_time[t],
        #                 to_units=pyunits.dimensionless)
        #         ) >= 0.2 * pyunits.mg / pyunits.L
        #     )
...
def build_sweep_params(model):
    sweep_params = dict()

    # Feed volumetric flow is fixed at fs.feed.flow_vol[0]
    sweep_params["System Capacity (MGD)"] = LinearSample(
        model.fs.feed.flow_vol[0],
        lower_limit=2 * pyunits.Mgallons / pyunits.day,
        upper_limit=40 * pyunits.Mgallons / pyunits.day,
        num_samples=5
    )
    return sweep_params
...
def save_outputs(m):
    return {
        "LCOW": value(pyunits.convert(m.fs.LCOW, to_units=pyunits.USD_2020 / pyunits.m**3)),
        "LCOT": value(pyunits.convert(m.fs.LCOT, to_units=pyunits.USD_2020 / pyunits.m**3)),
        "Capex (MUSD)": value(pyunits.convert(m.fs.total_capital_cost, to_units=pyunits.MUSD_2020)),
        "Opex (MUSD/year)": value(pyunits.convert(m.fs.total_operating_cost, to_units=pyunits.MUSD_2020 / pyunits.year)),
        "Specific Energy (kWh/m3)": value(m.fs.specific_energy_intensity),
    }

if __name__ == "__main__":
    model, results = main(working_directory="local")

    # sweep_params = build_sweep_params(m)
    #
    # outputs = parameter_sweep(
    #     m=m,
    #     sweep_params=sweep_params,
    #     outputs=save_outputs,
    #     results_filename="nonRO_parameter_sweep_results.csv",
    #     csv_results_file=True,
    #     hdf5_results_file=False,
    # )