...
def main(working_directory=None):
    ...
    LRV_req, details, state, treatment_train, effluent_type, solute_list = DPR_initial_setting(
        state="FL", treatment_train="CBAT", effluent_type="TERTIARY"
    )
...
def DPR_initial_setting(state=None, treatment_train=None, effluent_type=None):  #In the future, we might need to consider another type of tratment_train for Secondary effluent (for nutrient removal)
    ...
    solute_list = {
        "TERTIARY": {
            ...
            "nitrate": 0.01 * pyunits.kg / pyunits.m ** 3,
            "nitrite": 0.01 * pyunits.kg / pyunits.m ** 3,
        },
        "SECONDARY": {
...
# res, details, state, treatment_train, effluent_type, solute = DPR_initial_setting(state="FL", treatment_train="CBAT", effluent_type="TERTIARY")
...
def set_operating_conditions(model, solute_list=None, treatment_train=None):
    ...
    feed_flow_vol = 10 * 0.04381 * pyunits.m ** 3 / pyunits.s  # 1 MGD = 0.04381 m3/s
...
def display_cost(m, treatment_train=None):
    if treatment_train == "CBAT":
        ...
        Ozone_opex = value(
            pyunits.convert(m.fs.non_RO.Ozone.electricity[
